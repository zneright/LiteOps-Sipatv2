...
export default function Sidebar({ isOpen, onClose }: SidebarProps) {
  ...
  return (
    <>
      {/* Mobile Overlay */}
      <div
        className={`fixed inset-0 bg-slate-900/40 dark:bg-black/60 backdrop-blur-sm z-40 transition-opacity duration-300 lg:hidden ${
      ...
      {/* Sidebar Content */}
      <aside
        className={`fixed lg:sticky top-0 lg:top-16 left-0 z-50 h-full lg:h-[calc(100vh-4rem)] w-72 bg-white/80 dark:bg-slate-950/80 backdrop-blur-2xl border-r border-slate-200/60 dark:border-slate-800/60 flex flex-col transform transition-all duration-300 ease-in-out ${
            ...
            {visibleItems.map((item) => {
              ...
              return (
                <Link
                  ...
                  className={`relative flex items-center gap-3 px-4 py-3.5 rounded-2xl text-sm font-bold transition-all duration-300 overflow-hidden group ${
                    isActive
                ...
                >
                  ...
                  <svg
                    className={`w-5 h-5 shrink-0 transition-colors relative z-10 ${
