...
export default function SystemLogs() {
    ...
    const handleExport = () => {
        ...
        // A slight delay allows the modal to close visually before the browser freezes for the print dialog
        setTimeout(() => {
            window.print();
        }, 150);
    };

    // --- Dynamic Native Print CSS ---
    // This tells the browser exactly how to format the PDF without needing a library!
    const printStyles = `
        @media print {
            @page {
    ...
    return (
        <div className="w-full max-w-5xl mx-auto flex flex-col gap-8 pb-20 pt-4 px-4 sm:px-6 print:p-0 print:m-0 print:max-w-none print:bg-white">

            {/* Inject dynamic print styles into the DOM */}
            <style>{printStyles}</style>

            {/* --- Document Header (Visible ONLY in the PDF/Printout) --- */}
            <div className="hidden print:flex flex-col items-center justify-center border-b pb-6 mb-6 border-slate-200">
                ...
                <h1 className="text-3xl font-black text-slate-900 m-0">System Audit Logs</h1>
            ...
            {/* --- Web App Header (Hidden during PDF/Print) --- */}
            ...
            {/* --- Filter Toolbar (Hidden during PDF/Print) --- */}
            ...
            <div className="bg-white rounded-[2rem] border border-slate-200 shadow-sm p-6 sm:p-10 min-h-[500px] print:border-none print:shadow-none print:p-0">
                {isLoading ? (
                    <div className="py-20 flex justify-center print:hidden"><div className="animate-spin h-10 w-10 border-4 border-purple-600 border-t-transparent rounded-full"></div></div>
                ) : displayedLogs.length === 0 ? (
                    <div className="py-32 flex flex-col items-center text-center">
                        <span className="text-6xl mb-4 block opacity-50">📭</span>
                        <h3 className="text-xl font-black text-slate-900">No activity found.</h3>
                        <p className="text-sm text-slate-500 print:hidden">Adjust your filters or try a different query.</p>
                    </div>
                ) : (
                    <div className="relative border-l-2 border-slate-100 ml-4 sm:ml-8 space-y-8 pb-4 print:border-slate-300 print:ml-4">
                        {displayedLogs.map((log, i) => {
                            ...
                            return (
                                <motion.div
                                    ...
                                    key={log.id || i}
                                    className="relative pl-6 sm:pl-8 print:break-inside-avoid print:pl-6"
                                >
                                    {/* Timeline Node */}
                                    <div className={`absolute -left-[17px] top-1 w-8 h-8 rounded-full border-4 border-white flex items-center justify-center text-xs shadow-sm ${ui.color} print:border-white print:shadow-none`}>
                                        {ui.icon}
                                    </div>

                                    {/* Card Content */}
                                    <div className={`transition-colors p-5 rounded-2xl border ${isAnonymized ? 'bg-rose-50/50 border-rose-100' : 'bg-slate-50 hover:bg-slate-100 border-slate-100'} print:bg-transparent print:border-b print:border-x-0 print:border-t-0 print:rounded-none print:px-0 print:py-4`}>
                                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-2">
                                            <div className="flex items-center gap-2 flex-wrap">
                                                ...
                                                <span className={`px-2 py-0.5 text-[9px] font-black uppercase tracking-widest rounded-md print:border print:border-slate-300 print:bg-white ${ui.color.split(' ')[0]} bg-white bg-opacity-50`}>{log.action_type}</span>
                    ...
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm print:hidden">
                        ...
                        >
                            <div className="p-6 border-b border-slate-100">
                                <h2 className="text-xl font-black text-slate-900">Export Settings</h2>
                                <p className="text-sm text-slate-500">Configure your document before saving as PDF.</p>
                            </div>

                            <div className="p-6 space-y-5 bg-slate-50">
                                {/* Header Toggle */}
                                <label className="flex items-center justify-between cursor-pointer">
                                    <span className="text-sm font-bold text-slate-700">Include Document Header</span>
                                    <input
                                        type="checkbox"
                            ...
                            <div className="p-4 bg-white flex flex-col gap-3">
                                ...
                                <p className="text-[10px] text-center text-slate-400 font-medium px-2">
                                    When the print dialog opens, set the destination to <strong className="text-slate-600">"Save as PDF"</strong> to download the file.
                                </p>
                            </div>
                        </motion.div>
                    </div>
