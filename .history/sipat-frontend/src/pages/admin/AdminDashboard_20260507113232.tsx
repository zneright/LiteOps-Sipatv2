...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    const fetchAdminStats = async () => {
      try {
        ...
        // Calculate Global Reactions across all projects
        ...
        // Sort top global projects
        enrichedProjects.sort((a, b) => b.totalEngagement - a.totalEngagement);

        // Generate Recent Registrations
        const recentUsers = [...allUsers]
          .sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime())
          .slice(0, 4);

        // Set the massive state object
        setMetrics({
          ...
          totalAiScans: totalComments, // AI scans every comment!
          systemUptime: "99.99%",
        });

        setRecentLogs(recentUsers);
        setTopGlobalProjects(enrichedProjects.slice(0, 4)); // Top 4 for UI
  ...
  return (
    <div className="w-full max-w-7xl mx-auto flex flex-col gap-8 pb-20 pt-4 px-4 sm:px-6">
      ...
      {/* 🚀 GLOBAL MANAGEMENT PANELS */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mt-2">

        {/* Global Recent Registrations */}
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800/60 rounded-[2.5rem] p-8 shadow-sm">
          ...
          <Link to="/explore" className="inline-block mt-6 text-xs font-black uppercase tracking-widest text-amber-600 dark:text-amber-400 hover:text-amber-700 transition-colors">
