...
export default function AdminDashboard() {
  ...
  useEffect(() => 
    const fetchAdminStats = asyc () => {
      try 
        const [usersRes, projecsRes] = await Promise.all([
          fetch("http://localhot:8080/api/users"),
          fetch("http://localhot:8080/api/projects")
        ])
    ...
    <div className="w-full max-w-7xl mx-auto flex flex-col gap-8 pb-20 pt-4 px-4 sm:px-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mt-2">
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800/60 rounded-[2.5rem] p-8 shadow-sm">
          ...
          <Link to="/admin/logs" className="inline-block mt-6 text-xs font-black uppercase tracking-widest text-purple-600 dark:text-purple-400 hover:text-purple-700 transition-colors">
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800/60 rounded-[2.5rem] p-8 shadow-sm">
          ...
          <div className="space-y-4">
            {topGlobalProjects.length > 0 ? topGlobalProjects.map((p, index) => (
              <Link key={index} to={`/admin/project/${p.id}`} className="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-950/50 rounded-2xl border border-slate-100 dark:border-slate-800/60 group hover:border-amber-200 dark:hover:border-amber-500/30 transition-colors">
                <div className="flex items-center gap-4">
