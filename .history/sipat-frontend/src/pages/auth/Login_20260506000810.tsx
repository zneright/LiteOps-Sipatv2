...
export default function Login() {
  ...
  // --- HELPER FUNCTION TO FETCH ROLE FROM PHP BACKEND ---
  const fetchUserRole = async (uid: string) => {
    try {
      ...
      // 🐛 DEBUGGING: Open your browser console (F12) to see exactly what PHP sends back!
      console.log("PHP Database Response:", responseData);

      // SCENARIO 1: PHP returns an array (e.g., [ { role: "agency", email: "..." } ])
      if (Array.isArray(responseData) && responseData.length > 0) {
        return responseData[0].role || "citizen";
      }

      // SCENARIO 2: PHP wraps the data in a 'data' object (e.g., { data: { role: "agency" } })
      if (responseData.data && responseData.data.role) {
        return responseData.data.role;
      }

      // SCENARIO 3: PHP returns a flat object (e.g., { role: "agency" })
      return responseData.role || "citizen";
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 1. Fetch their real role from your newly fixed PHP backend
      let finalRole = await fetchUserRole(user.uid);

      // 2. HARDCODE OVERRIDE: If it is your master email, force Super Admin role!
      if (user.email === "pawpoint2024@gmail.com") {
        finalRole = "admin";
      }

      // 3. Log them in and Route them
      login(user.email || "", finalRole);
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      // 1. Fetch their real role from the DB
      let finalRole = await fetchUserRole(user.uid);

      // 2. HARDCODE OVERRIDE for Google logins too!
      if (user.email === "pawpoint2024@gmail.com") {
        finalRole = "admin";
      }
