...
export default function Login() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let finalName = (finalRole === "agency" && dbData?.organization_name)
        ? dbData.organization_name
        : (dbData?.full_name || user.displayName || "Citizen");
      ...
      fetch("http://localhost:8080/api/logs/create", {
        ...
        body: JSON.stringify({
          ...
          action_type: "LOGIN",
          description: "Successfully authenticated and logged into the platform."
        })
      }).catch(err => console.log("Log error:", err)); // Fails silently so it doesn't interrupt UX
    } catch (error: any) {
