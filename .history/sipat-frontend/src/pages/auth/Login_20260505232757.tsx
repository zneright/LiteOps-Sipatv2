...
export default function Login() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (user.email === "admin@dpwh.gov.ph") {
        assignedRole = "agency";
      } else if (user.email === "pawpoint2024@gmail.com") {
        assignedRole = "admin";
      }
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      // --- NEW: CHECK IF THEY ARE REGISTERED ---
      const additionalInfo = getAdditionalUserInfo(userCredential);

      if (additionalInfo?.isNewUser) {
        // They are trying to log in but don't exist yet! 
        // We delete the accidentally created Firebase account and throw an error.
        await user.delete();
        await auth.signOut();
      ...
      // If they exist, continue logging them in...
      let assignedRole: "citizen" | "agency" | "admin" = "citizen";
