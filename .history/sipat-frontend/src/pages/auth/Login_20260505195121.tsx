...
export default function Login() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2. Ask CodeIgniter for their REAL role
      const response = await fetch(`http://localhost:8080/api/users/${encodeURIComponent(user.email || '')}`);

      if (!response.ok) {
        throw new Error("User found in Firebase but missing in Database. Please Sign Up.");
      }

      const dbUser = await response.json();
      const assignedRole = dbUser.role; // Gets 'citizen', 'agency', or 'admin' from your DB!

      // 3. Log them in and dynamically route!
      login(user.email || "", assignedRole);
      navigate(`/${assignedRole}/dashboard`); // Routes to /citizen/dashboard, /admin/dashboard, etc.
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      // Fetch their real role from CodeIgniter
      const response = await fetch(`http://localhost:8080/api/users/${user.email}`);

      if (!response.ok) {
        await auth.signOut();
        setAuthError("Account not synced with database. Please contact support.");
        setIsLoading(false);
        return;
      }

      const dbUser = await response.json();
      const assignedRole = dbUser.role;
