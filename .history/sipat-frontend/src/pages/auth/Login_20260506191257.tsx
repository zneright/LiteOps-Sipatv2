...
export default function Login() {
  ...
  // --- 🚀 UPDATED: HELPER TO FETCH FULL USER DATA FROM PHP ---
  const fetchUserData = async (uid: string) => {
    try {
      ...
      console.log("PHP Database Response:", responseData);

      // Return the full object based on how your PHP structures the response
      ...
      return null; // Return null so we can use fallbacks in the login functions
    }
  };
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 1. Fetch their full data from your PHP backend
      const dbData = await fetchUserData(user.uid);

      // 2. Extract values with safe fallbacks
      let finalRole = dbData?.role || "citizen";
      // ✅ Fixed Code
      let finalName = dbData?.full_name || user.displayName || "Citizen";

      // 3. HARDCODE OVERRIDE: If it is your master email, force Super Admin role!
      if (user.email === "pawpoint2024@gmail.com") {
        finalRole = "admin";
      }

      // 4. 🚀 Pass the FULL object to the context login function
      login({
        email: user.email || "",
        role: finalRole,
        name: finalName,
        id: dbData?.id // Pass the DB ID if your backend provided it
      });
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (additionalInfo?.isNewUser) {
        // They are trying to log in but don't exist yet! 
        await user.delete();
        await auth.signOut();
      ...
      // 1. Fetch their full data from the DB
      const dbData = await fetchUserData(user.uid);

      // 2. Extract values with safe fallbacks (Google provides a displayName we can fallback to!)
      let finalRole = dbData?.role || "citizen";
      let finalName = dbData?.name || user.displayName || "Citizen";

      // 3. HARDCODE OVERRIDE for Google logins too!
      if (user.email === "pawpoint2024@gmail.com") {
        finalRole = "admin";
      }

      // 4. 🚀 Route them using the full user object
      login({
        email: user.email || "",
        role: finalRole,
        name: finalName,
