...
export default function Signup() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        await user.delete(); // Rollback if DB fails
        throw new Error("Failed to save to database.");
      }

      login(user.email || "", role);
      navigate(role === "agency" ? "/agency/dashboard" : "/home");
  ...
  const handleGoogleSignup = async () => {
    // If they want to be an agency, we still need their org name!
    ...
    try {
      ...
      // Check if they are actually a new user
      const additionalInfo = getAdditionalUserInfo(userCredential);

      if (additionalInfo?.isNewUser) {
        // Prepare Data for PHP Database
        const userData = {
          firebase_uid: user.uid,
          email: user.email,
          full_name: user.displayName || "Google User", // Google provides their real name
          role: role,
          organization_name: role === "agency" ? organization : null,
          is_approved: role === "citizen" ? 1 : 0
        };
        ...
        if (!response.ok) {
          await user.delete(); // Rollback if backend fails
