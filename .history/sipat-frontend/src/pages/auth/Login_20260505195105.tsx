...
export default function Login() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2. Role Check (In a real app, you would fetch this from your CodeIgniter DB)
      // For now, we use the email identifiers we set up
      let assignedRole: "citizen" | "agency" | "admin" = "citizen";
      
      // Update the agency email if you have a specific one, but change the admin email to yours!
      if (user.email === "admin@dpwh.gov.ph") {
        assignedRole = "agency";
      } else if (user.email === "pawpoint2024@gmail.com") { // <--- CHANGED THIS LINE
        assignedRole = "admin";
      }

      // 3. Log them in via Context
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      // Google logins default to Citizen role
