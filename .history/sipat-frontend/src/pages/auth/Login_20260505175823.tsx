...
export default function Login() {
  ...
  // Demo Mode State
  const [demoMode, setDemoMode] = useState<"citizen" | "agency" | "admin" | null>(null);
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    // Determine the actual credentials to use (User input OR Demo Mode placeholders)
    let targetEmail = email;
    let targetPassword = password;

    if (!email && demoMode) {
      if (demoMode === "citizen") {
        targetEmail = "juan.delacruz@gmail.com";
        targetPassword = "citizen_demo_2026";
      } else if (demoMode === "agency") {
        targetEmail = "admin@dpwh.gov.ph";
        targetPassword = "agency_secure_pass_2026";
      } else if (demoMode === "admin") {
        targetEmail = "superadmin@sipat.ph";
        targetPassword = "master_override_2026";
      }
    } else if (!email && !demoMode) {
      setAuthError("Please enter your email or select a Demo Fast Track.");
      setIsLoading(false);
      return;
    }
  ...
  const handleDemoModeSelect = (role: "citizen" | "agency" | "admin") => {
    setDemoMode(role);
    setEmail(""); // Ensure inputs are empty so placeholders show
    setPassword("");
    setAuthError("");
  };

  // Dynamic Placeholders based on Demo Mode
  const getEmailPlaceholder = () => {
    if (demoMode === "citizen") return "juan.delacruz@gmail.com";
    if (demoMode === "agency") return "admin@dpwh.gov.ph";
    if (demoMode === "admin") return "superadmin@sipat.ph";
    return "Enter your email";
  };

  const getPasswordPlaceholder = () => {
    if (demoMode) return "•••••••• (Demo Ready)";
    return "••••••••";
  };
        ...
        >
            ...
            <div>
              ...
              <div className="relative">
                <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                  <svg className={`h-5 w-5 transition-colors ${demoMode ? 'text-indigo-500' : 'text-slate-400'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    setDemoMode(null); // Clear demo mode if they start typing naturally
                  }}
                  className={`w-full pl-11 pr-4 py-3.5 rounded-xl bg-white/50 dark:bg-slate-950/50 border ${demoMode ? 'border-indigo-400/50 dark:border-indigo-500/50 bg-indigo-50/50 dark:bg-indigo-900/10' : 'border-slate-200 dark:border-slate-800'} text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500 transition-all font-medium`}
          ...
          <div className="mt-8 pt-6 border-t border-slate-200/60 dark:border-slate-700/60 relative z-10">
            ...
            <div className="grid grid-cols-3 gap-2">
              <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => handleDemoModeSelect("citizen")} type="button" className={`p-3 rounded-xl text-xs font-bold transition-all flex flex-col items-center gap-1 border ${demoMode === "citizen" ? "bg-emerald-500/20 text-emerald-700 dark:text-emerald-400 border-emerald-500/50 shadow-inner" : "bg-emerald-500/5 hover:bg-emerald-500/10 text-emerald-600/70 dark:text-emerald-500/70 border-emerald-500/10"}`}>
                <svg className="w-5 h-5 mb-1" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" /></svg>
                Citizen
              </motion.button>
              <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => handleDemoModeSelect("agency")} type="button" className={`p-3 rounded-xl text-xs font-bold transition-all flex flex-col items-center gap-1 border ${demoMode === "agency" ? "bg-blue-500/20 text-blue-700 dark:text-blue-400 border-blue-500/50 shadow-inner" : "bg-blue-500/5 hover:bg-blue-500/10 text-blue-600/70 dark:text-blue-500/70 border-blue-500/10"}`}>
              ...
              <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => handleDemoModeSelect("admin")} type="button" className={`p-3 rounded-xl text-xs font-bold transition-all flex flex-col items-center gap-1 border ${demoMode === "admin" ? "bg-purple-500/20 text-purple-700 dark:text-purple-400 border-purple-500/50 shadow-inner" : "bg-purple-500/5 hover:bg-purple-500/10 text-purple-600/70 dark:text-purple-500/70 border-purple-500/10"}`}>
