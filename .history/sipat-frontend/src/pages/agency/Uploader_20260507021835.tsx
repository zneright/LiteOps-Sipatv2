...
export default function Uploader() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 🚀 THE FIX: Attach the currently logged in Agency's name!
      const projectData = {
        title,
        category,
        budget,
        ...
        file_type: file.type,
        organization_name: user?.name // 👈 THIS IS CRITICAL
      };
  ...
  const analyzeWithAI = async (selectedFile: File) => {
    ...
    try {
      ...
      // 1. Define the EXACT structure with REQUIRED fields
        ...
        properties: {
          ...
          phases: {
            ...
            items: {
              ...
              // 🚀 FIX: Force the AI to fill out every field inside a phase
              required: ["title", "description", "start_date", "completion_date", "image_url"]
            }
          }
        },
        // 🚀 FIX: Force the AI to always return the phases array, even if it only finds one phase
        required: ["title", "description", "budget", "phases"]
      };

      const model = genAI.getGenerativeModel({
        model: "gemini-2.5-flash-lite", // or "gemini-1.5-flash" if lite struggles with extraction
        generationConfig: {
      ...
      // 🚀 FIX: Give the AI better context on what a "phase" is
      const prompt = `
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <select value={category} onChange={(e) => setCategory(e.target.value)} className="w-full px-4 py-3 rounded-xl bg-slate-50 border border-slate-200 focus:ring-2 focus:ring-indigo-500/50">
              <option>Infrastructure</option><option>Technology</option><option>Environment</option><option>Healthcare</option><option>Education</option><option>Transport</option>
            </select>
          </div>
          <div>
