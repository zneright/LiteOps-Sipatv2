...
// 🚀 Updated Interface to include parent_id and target_phase for threading/grouping
interface Comment {
    id: number;
    project_id: number;
...
export default function AgencyFeedback() {
    ...
    // 🚀 NEW: Recursive function to render threaded comments inside the Dashboard!
        ...
        return (
            <div key={comment.id} className="relative">
                ...
                `}>
                    ...
                    {/* 🚀 Reactions Display directly in the Dashboard */}
        ...
        <div className="w-full max-w-5xl mx-auto px-4 py-8 flex flex-col gap-8">
            ...
            <div className="flex flex-col gap-6">
                {projects.map((project, idx) => {
                    ...
                    // 🚀 NEW: Group the comments by Phase for this specific project
                    const uniquePhases = Array.from(new Set(stats.comments.map(c => c.target_phase || "Overall Project")));
                    ...
                    return (
                        ...
                        >
                            ...
                            {/* Expanded Comments Section grouped by Phase */}
                            <AnimatePresence>
                                {isExpanded && (
                                    <motion.div
                                    ...
                                    >
                                        <div className="p-6 md:p-8 flex flex-col gap-8">
                                            {stats.comments.length === 0 ? (
                                                ...
                                                // 🚀 NEW: Render distinct sections for each Phase
                                                uniquePhases.map(phase => {
                                                    const phaseComments = stats.comments.filter(c => (c.target_phase || "Overall Project") === phase);
                                                    ...
                                                    return (
                                                        <div key={phase} className="flex flex-col gap-4">
                                                            ...
                                                            <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-700 pb-2">
                                                                <h4 className="font-black text-sm text-slate-800 dark:text-slate-200 uppercase tracking-wide">
                                                                    {phase}
                                                                </h4>
                                                            ...
                                                            <div className="flex flex-col gap-6">
                                                                {topLevelComments.length === 0 ? (
                                                                    <p className="text-xs text-slate-400 italic">No direct feedback for this phase.</p>
                                                                ) : (
                                                                    topLevelComments.map(comment => renderComment(comment, phaseComments, false))
                                                                )}
                                                            </div>
                                                        </div>
                                                    );
