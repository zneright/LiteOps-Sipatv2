...
export default function AgencyProfile() {
    ...
    useEffect(() => {
        // Wait for the AuthContext to load the user
        if (!user) return;

        const fetchDashboardData = async () => {
            try {
                // 🚀 FIX 1: Catch all possible ID variations from your AuthContext
                const f_uid = user.uid || user.firebase_uid || user.id || null;

                let dbUser: any = {};

                // Only try to fetch the database user if we successfully found an ID
                if (f_uid) {
                    ...
                    console.warn("⚠️ No ID found on user object. Using fallback data. User object:", user);
                }
                ...
                // Ensure DPWH and DB data overrides fallbacks
                ...
                // 🚀 FIX 2: This will ALWAYS run now, breaking the infinite loading screen!
                setIsLoading(false);
            }
        };
