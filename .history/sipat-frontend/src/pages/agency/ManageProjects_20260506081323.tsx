...
export default function ManageProjects() {
    ...
    const fetchProjects = async () => {
        try {
            ...
            // 🚀 THE FIX: Check if data is actually an array!
            if (Array.isArray(data)) {
                setProjects(data);
                setBackendError(null);
            } else {
                // If the backend sent an object (like a 500 error), catch it here!
                console.error("🚨 BACKEND ERROR:", data);
                setProjects([]);
                setBackendError(data.message || data.error || "Unknown backend error");
            }
        } catch (error) {
    ...
    return (
        <div className="w-full max-w-5xl mx-auto flex flex-col gap-8 pb-20 pt-4 px-4 text-slate-900 dark:text-white">
            ...
            {loading ? (
                ...
                <div className="bg-red-50 dark:bg-red-900/20 p-10 rounded-3xl border border-red-200 dark:border-red-800">
                    <h3 className="text-red-600 dark:text-red-400 font-black text-xl mb-2">Backend Error Detected</h3>
                    <p className="text-red-500 dark:text-red-300 font-medium">{backendError}</p>
                    <p className="text-sm mt-4 text-slate-500">Open your browser console (F12) for more details.</p>
                </div>
            ) : projects.length === 0 ? (
                            ...
                            <motion.div key={project.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[1.5rem] shadow-sm overflow-hidden">
                                ...
                                >
                                    <div className="flex-1">
                                        ...
                                        {typeof project.keywords === 'string' && project.keywords && (
                                        ...
                                        >
                                            <div className="p-6 bg-slate-50/50 dark:bg-slate-950/30">
                                                ...
                                                {parsedPhases.length === 0 ? (
                                                    <div className="text-center p-4 text-slate-400 font-medium">No timeline data extracted.</div>
                                                ) : (
