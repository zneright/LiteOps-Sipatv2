...
export default function AgencyProfile() {
    ...
    // 🚀 NEW: Store the real Firebase UID from the database
    ...
    useEffect(() => {
        const fetchDashboardData = async () => {
            // 🚀 THE FIX: Use the email to fetch data, since we know it exists!
            if (!user?.email) return;

            try {
                // 1. Fetch Agency Profile from DB using EMAIL
                ...
                if (uRes.ok) {
                    ...
                    // 🚀 Save the REAL Firebase UID for the save button
                    if (dbUser.firebase_uid) {
                        setRealUid(dbUser.firebase_uid);
                    }
                }
    ...
    const handleSave = async () => {
        ...
        // 🚀 THE FIX: Use the REAL UID from the database, not the context
        if (!realUid) {
