...
export default function Uploader() {
  ...
  const analyzeWithAI = async (selectedFile: File) => {
    ...
    try {
      ...
      // Gemini 1.5 Flash is incredibly fast and natively supports PDFs
      const model = genAI.getGenerativeModel({ model: "gemini-1.5-flash" });

      const prompt = `
        You are an elite AI assistant for a Government Transparency Platform. 
        Read this official project document/PDF. Extract the core details and return them STRICTLY as a JSON object. 
        Do not include any markdown styling, just raw JSON.
        
        Keys required:
        - "title": The official name of the project.
        - "category": Choose the closest match from: ["Infrastructure", "Technology", "Environment", "Healthcare"]. If none match, default to "Infrastructure".
        - "budget": Extract the exact total budget allocated. Return only the numbers and commas (e.g., "5,000,000"). If none found, leave empty.
        - "description": A concise, highly specific 2-sentence summary of the project scope and its community impact. No fluff.
      `;
      ...
      const cleanJsonStr = responseText.replace(/```json/g, '').replace(/
        ```/g, '').trim();
