...
export default function AgencyFeedback() {
    ...
    useEffect(() => {
        // Only run fetch if the user is loaded
        if (user?.name) {
            const fetchAgencyData = async () => {
                try {
                    ...
                    // 🚀 THE FIX: Filter projects to ONLY show this agency's projects!
                    const agencyProjects = data.filter((p: Project) =>
                        p.organization_name === user?.name ||
                        (!p.organization_name && user?.name === "DPWH")
                    );

                    // Fetch comments ONLY for this agency's projects
    ...
    return (
        <div className="w-full max-w-5xl mx-auto px-4 py-8 flex flex-col gap-8">
            ...
            {projects.length === 0 ? (
                <div className="bg-white dark:bg-slate-900 p-10 rounded-[2rem] text-center border border-slate-200 dark:border-slate-800 shadow-sm">
                    <span className="text-4xl mb-4 block">📭</span>
                    <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-2">No projects found</h3>
                    <p className="text-slate-500 text-sm">You haven't published any projects to receive feedback on yet.</p>
                </div>
            ) : (
