...
export default function AgencyFeedback() {
    ...
    useEffect(() => {
        const fetchAgencyData = async () => {
            try {
                ...
                // Fetch ALL Comments to calculate insights
    ...
    // 🚀 Calculate Good vs Bad based on saved AI data
    const getProjectAnalytics = (projectId: number) => {
        ...
        // BAD: AI flagged it as a Ghost Project (Handling the string "0" / "1" issue)
        ...
        // NEUTRAL: Low match score, but not flagged as a ghost project
                                ...
                                <div className="w-full md:w-72 flex flex-col gap-2 shrink-0">
                                    ...
                                    >
                                        {isExpanded ? (
                                            ...
                                            <>View AI Analysis <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M19 9l-7 7-7-7" /></svg></>
                                        )}
                                    </button>
                                </div>
                            </div>
                                            ...
                                            {stats.comments.length === 0 ? (
                                                ...
                                                <div className="flex flex-col gap-4">
                                                    {stats.comments.map(comment => {
                                                        ...
                                                        return (
                                                            <div key={comment.id} className={`p-5 rounded-2xl border shadow-sm flex flex-col md:flex-row gap-4 items-start justify-between ${isGhost ? 'bg-red-50 dark:bg-red-500/10 border-red-200' : comment.ai_match_score >= 50 ? 'bg-emerald-50 dark:bg-emerald-500/10 border-emerald-200' : 'bg-white dark:bg-slate-950 border-slate-200 dark:border-slate-800'}`}>
                                                                <div>
                                                                    ...
                                                                    <p className="text-sm text-slate-600 dark:text-slate-300">{comment.text_content}</p>
                                                                </div>
                                                                <Link to={`/project/${project.id}`} className="text-xs font-bold text-indigo-600 hover:text-indigo-800 shrink-0 flex items-center gap-1 mt-2 md:mt-0 bg-white dark:bg-slate-800 px-3 py-1.5 rounded-lg border border-slate-200 dark:border-slate-700 shadow-sm transition-colors">
                                                                    View Context
