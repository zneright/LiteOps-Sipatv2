...
export default function Uploader() {
  ...
  const analyzeWithAI = async (selectedFile: File) => {
    ...
    try {
      ...
      // 🚀 THE FIX: Use the actual Gemini 1.5 Pro model, which perfectly reads PDFs with images inside!
      const model = genAI.getGenerativeModel({ model: "gemini-1.5-pro" });

      const prompt = `
        You are a highly secure, objective Data Extraction AI for a Government Transparency Platform.
        Analyze the attached official public document (including any images or charts inside it). Extract ONLY the factual data requested. Do not hallucinate or guess. If a value is not explicitly stated in the document, leave it empty.
