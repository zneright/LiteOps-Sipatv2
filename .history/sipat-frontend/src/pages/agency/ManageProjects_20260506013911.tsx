...
interface Project {
    ...
    phases: string;
    category: string;
    file_url?: string; // 🚀 NEW: Added file_url so we can view the PDF!
}
...
export default function ManageProjects() {
    ...
    // Manual verification (No Photo needed, usually verified by citizens)
    const handleManualVerify = async (projectId: number, phaseIndex: number) => {
        try {
            ...
            // If unchecking, also clear the image just in case
    ...
    // Helper to save to DB and update UI
                            ...
                            <motion.div key={project.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[1.5rem] shadow-sm overflow-hidden">
                                ...
                                >
                                    <div className="flex-1">
                                        ...
                                        <p className="text-slate-500 dark:text-slate-400 font-bold text-sm mt-1">Budget: ₱{project.budget}</p>
                                    </div>
                                    ...
                                    {isExpanded && (
                                        ...
                                        >
                                            <div className="p-6 bg-slate-50/50 dark:bg-slate-950/30">
                                                ...
                                                {/* 🚀 NEW: Header Row with Title AND PDF Button */}
                                                <div className="flex flex-wrap justify-between items-center gap-4 mb-6">
                                                    ...
                                                    {project.file_url && (
                                                        <a
                                                            ...
                                                            className="flex items-center gap-2 px-3 py-1.5 bg-blue-50 hover:bg-blue-100 dark:bg-blue-500/10 dark:hover:bg-blue-500/20 text-blue-600 dark:text-blue-400 text-xs font-bold rounded-lg transition-colors border border-blue-200 dark:border-blue-500/30 shadow-sm"
                                                            onClick={(e) => e.stopPropagation()} // Prevents the accordion from closing when clicked
                                                        >
                                                                ...
                                                                <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] p-4 rounded-xl border bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700 shadow-sm transition-all hover:shadow-md text-slate-900 dark:text-slate-100">
                                                                    <div className="flex justify-between items-start mb-2">
                                                                        ...
                                                                        <button
                                                                            ...
                                                                            className={`text-[10px] font-bold px-2 py-1 rounded border transition-colors ${isCompleted ? 'bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-200 dark:border-emerald-500/30' : 'bg-slate-50 dark:bg-slate-800 text-slate-500 border-slate-200 dark:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-700'}`}
                                                                    ...
                                                                    <p className="text-xs font-medium text-slate-500 dark:text-slate-400 mb-4">{phase.description}</p>
