...
export default function AgencyAnalytics() {
    ...
    if (isLoading) return (
        <div className="w-full h-[60vh] flex flex-col items-center justify-center gap-4">
            ...
            <p className="text-slate-400 dark:text-slate-500 font-bold tracking-widest uppercase text-xs">Compiling Global Analytics...</p>
        </div>
    );
    ...
    return (
        <div className="w-full max-w-6xl mx-auto flex flex-col gap-6 sm:gap-8 pb-20 px-4 sm:px-6 pt-4 sm:pt-8 text-slate-900 dark:text-slate-100">
            <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5 }}>
                <h1 className="text-3xl sm:text-4xl font-black tracking-tight mb-2">Global <span className="text-indigo-600 dark:text-indigo-400">Analytics</span></h1>
                                ...
                                {detailedProjects.map((p) => (
                                    <tr key={p.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors group">
                                        ...
                                        <td className="py-4 px-6 text-right">
                                            {/* Adjusted opacity so mobile users can still see it without hover */}
