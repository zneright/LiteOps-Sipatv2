    ...
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        const commentsPromises = myProjects.map((p: Project) =>
          fetch(`${API_URL}/api/api/comments/${p.id}`).then(res => res.json())
        );
    ...
    >
      ...
      <div className="w-full mt-2">
        <motion.div
          ...
          className="w-full bg-white dark:bg-slate-900/80 backdrop-blur-xl border border-slate-200/60 dark:border-slate-800/60 rounded-[2rem] shadow-xl shadow-slate-200/20 dark:shadow-none flex flex-col overflow-hidden"
        >
