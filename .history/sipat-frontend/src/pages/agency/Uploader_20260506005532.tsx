...
export default function Uploader() {
  ...
  const [phases, setPhases] = useState<string[]>([]); // 🚀 NEW: Array to hold the steps
  ...
  // ⚠️ REMEMBER: Delete this key from AI Studio and make a new one after the hackathon!
  ...
  const analyzeWithAI = async (selectedFile: File) => {
    ...
    try {
      ...
      const prompt = `
        ...
        Analyze the attached official public document. Extract ONLY the factual data requested. Do not hallucinate.
        ...
        - "budget": Extract the total allocated budget amount. Return only numbers and commas.
        - "description": A short, 1-2 sentence overview for the public feed.
        - "ai_summary": A detailed, comprehensive text extraction of the project's goals, timelines, contractors, and specific location data.
        - "phases": Create a chronological Array of Strings representing the lifecycle steps or milestones mentioned (e.g. ["Bidding", "Contract Awarded", "Material Procurement", "Construction"]). Limit to 5 major phases max. If none are found, return an empty array [].
      `;
      ...
      const cleanJsonStr = responseText.replace(/```json/gi, '').replace(/
        ```/g, '').trim();
      ...
      if (aiData.phases && Array.isArray(aiData.phases)) setPhases(aiData.phases); // 🚀 Set Phases!
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const projectData = {
        ...
        phases: JSON.stringify(phases), // 🚀 Convert array to JSON string for MySQL
    ...
    <div className="w-full max-w-4xl mx-auto flex flex-col gap-8 pb-20 pt-4 px-4">
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          {phases.length > 0 && (
            <div className="md:col-span-2 bg-indigo-50 dark:bg-indigo-500/5 border border-indigo-100 dark:border-indigo-500/20 p-4 rounded-xl">
              <label className="block text-xs font-bold text-indigo-700 dark:text-indigo-400 uppercase tracking-wider mb-3">Project Timeline (Extracted by AI)</label>
              <div className="flex flex-wrap gap-2">
                {phases.map((phase, index) => (
                  <div key={index} className="flex items-center gap-2">
                    <span className="px-3 py-1.5 bg-white dark:bg-slate-800 text-indigo-700 dark:text-indigo-300 text-xs font-bold rounded-lg border border-indigo-200 dark:border-indigo-500/30 shadow-sm">
                      {phase}
                    </span>
                    {index < phases.length - 1 && (
                      <svg className="w-4 h-4 text-indigo-300" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg>
                    )}
                  </div>
                ))}
              </div>
