...
export default function Profile() {
  ...
  // Safely grab whatever ID property your AuthContext uses
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // 3. Dynamically Calculate Rank based on contributions
        let calculatedRank = "Active Citizen";
        if (filtered.length >= 10) calculatedRank = "Lead Auditor";
        else if (filtered.length >= 5) calculatedRank = "Senior Auditor";
  ...
  const handleSaveProfile = async () => {
    // DIAGNOSTIC CHECK: If this triggers, your AuthContext is missing the user's ID
    if (!userId) {
      alert("Cannot save: User ID is missing. Check your AuthContext!");
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-center md:items-end gap-6">
          ...
          <div className="flex-1 text-center md:text-left pb-2">
            <div className="flex items-center justify-center md:justify-start gap-2">
              ...
              {/* Removed the Verified Citizen badge here */}
            </div>
            {details.location && (
            ...
            {details.bio && (
              <p className="text-xs text-indigo-500 font-bold mt-1 uppercase tracking-wider">
                {details.bio}
              </p>
            )}
          </div>
        ...
        {isEditing && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-white dark:bg-slate-900 p-8 rounded-[2rem] border border-slate-200 dark:border-slate-800 w-full max-w-md shadow-2xl">
              <h2 className="text-2xl font-black mb-6">Edit Profile Details</h2>
              <div className="flex flex-col gap-4">
                <div>
                  <label className="text-xs font-bold text-slate-400 uppercase ml-1">Location / Municipality</label>
                  <input type="text" placeholder="e.g. Obando, Bulacan" value={details.location} onChange={(e) => setDetails({ ...details, location: e.target.value })} className="w-full mt-1 p-3 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" />
                </div>
                <div>
                  <label className="text-xs font-bold text-slate-400 uppercase ml-1">Short Bio / Profession</label>
                  <textarea placeholder="e.g. Concerned Citizen | IT Professional" value={details.bio} onChange={(e) => setDetails({ ...details, bio: e.target.value })} className="w-full mt-1 p-3 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" rows={2} />
                </div>
                <div className="flex gap-3 mt-4">
                  <button onClick={() => setIsEditing(false)} disabled={isSaving} className="flex-1 py-3 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 rounded-xl font-bold text-sm disabled:opacity-50 transition-opacity">Cancel</button>
                  <button onClick={handleSaveProfile} disabled={isSaving} className="flex-1 py-3 bg-indigo-600 text-white rounded-xl font-bold text-sm shadow-lg shadow-indigo-500/20 disabled:opacity-50 transition-opacity">
