...
export default function ProjectView() {
    ...
    // Determine how many phases to show
    const visiblePhases = showAllPhases ? project.phases : project.phases?.slice(0, 3);
    const hasMorePhases = project.phases?.length > 3;
    ...
    const renderComment = (comment: any, isReply = false, parentName = "") => {
        ...
        return (
            <div key={comment.id} className="relative z-10">
                ...
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ type: "spring", stiffness: 300, damping: 25 }} className={` ${isReply ? 'ml-6 sm:ml-8 mt-4' : 'mt-8'} p-5 sm:p-6 rounded-[1.5rem] sm:rounded-[2rem] border transition-all duration-300 shadow-[0_4px_20px_rgb(0,0,0,0.03)] dark:shadow-[0_4px_20px_rgb(0,0,0,0.2)] hover:-translate-y-1 relative z-10 ${isGhost ? 'bg-rose-50/80 dark:bg-rose-900/10 border-rose-200/60 dark:border-rose-500/20' : comment.ai_match_score >= 40 ? 'bg-emerald-50/80 dark:bg-emerald-900/10 border-emerald-200/60 dark:border-emerald-500/20' : 'bg-white/80 dark:bg-slate-800/60 border-slate-100 dark:border-slate-700/50 backdrop-blur-xl'} `}>
                    ...
                    <div className="bg-white/50 dark:bg-slate-900/50 p-4 rounded-[1.25rem] border border-slate-100/50 dark:border-slate-800/50 shadow-inner mb-4">
                        <p className="text-slate-700 dark:text-slate-300 text-[15px] leading-relaxed font-medium">"{comment.text_content}"</p>
                    </div>
        ...
        <div className="w-full min-h-screen relative overflow-x-hidden font-sans bg-slate-50 dark:bg-slate-950 selection:bg-indigo-500/30 pb-20">
            ...
            <div className="relative max-w-[90rem] mx-auto py-8 sm:py-12 px-4 sm:px-6 lg:px-8 flex flex-col xl:flex-row gap-8 xl:gap-12">
                ...
                <div className="w-full xl:w-[45%] flex flex-col gap-8">
                    <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ type: "spring", bounce: 0.4 }} className="flex flex-col gap-8 xl:sticky xl:top-8">
                        <div>
                            ...
                            <div className="bg-white/60 dark:bg-slate-900/40 backdrop-blur-xl p-6 sm:p-8 rounded-[2rem] border border-white/50 dark:border-slate-700/50 shadow-sm relative">
                                ...
                                <p className="text-slate-700 dark:text-slate-300 text-[15px] leading-relaxed font-medium relative z-10 px-4 pt-4">{isOverall ? project.description : currentContextData?.description}</p>
                ...
                <div className="w-full xl:w-[55%] flex flex-col gap-8">
                    <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ type: "spring", bounce: 0.4 }} className="flex flex-col gap-8">
                        ...
                        {/* Top-to-Bottom Phases Selection */}
                        <div className="w-full flex flex-col gap-3">
                            ...
                            <button onClick={() => setActiveTab("Overall Project")} className={`relative flex items-center justify-between p-4 w-full rounded-[1.25rem] transition-all duration-300 transform-gpu overflow-hidden border ${activeTab === "Overall Project" ? "text-white dark:text-slate-900 border-transparent shadow-[0_8px_20px_rgba(99,102,241,0.3)] scale-[1.02] z-10" : "bg-white/70 dark:bg-slate-900/60 backdrop-blur-xl border-white/80 dark:border-slate-700/50 text-slate-500 dark:text-slate-400 hover:bg-white dark:hover:bg-slate-800 shadow-sm hover:shadow-md hover:-translate-y-0.5"}`}>
                                ...
                                <div className="flex items-center gap-4 relative z-10 w-full">
                                    <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 shadow-inner ${activeTab === "Overall Project" ? "bg-white/20 dark:bg-black/10 text-white dark:text-slate-900" : "bg-slate-100 dark:bg-slate-800 text-indigo-500"}`}>
                                    ...
                                    <div className="text-left flex-1 min-w-0">
                                        <span className={`block text-[10px] uppercase font-black tracking-widest mb-0.5 ${activeTab === "Overall Project" ? "opacity-80" : "text-slate-400"}`}>Primary Scope</span>
                                        <span className="block text-sm sm:text-base font-black leading-tight truncate">Overall Project</span>
                                    </div>
                                    <div className={`shrink-0 opacity-0 transform translate-x-[-10px] transition-all duration-300 ${activeTab === "Overall Project" ? "opacity-100 translate-x-0" : ""}`}>
                                        <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M9 5l7 7-7 7" /></svg>
                                    </div>
                                </div>
                            </button>

                            <div className="relative pl-6 sm:pl-8 ml-4 sm:ml-5 border-l-2 border-slate-200 dark:border-slate-700/50 flex flex-col gap-3">
                                {project.phases?.map((p: any, index: number) => {
                                    ...
                                    return (
                                        <button key={p.title} onClick={() => setActiveTab(p.title)} className={`relative flex items-center justify-between p-4 w-full rounded-[1.25rem] transition-all duration-300 transform-gpu overflow-hidden border ${isActive ? "text-white dark:text-slate-900 border-transparent shadow-[0_8px_20px_rgba(99,102,241,0.3)] scale-[1.02] z-10" : "bg-white/70 dark:bg-slate-900/60 backdrop-blur-xl border-white/80 dark:border-slate-700/50 text-slate-500 dark:text-slate-400 hover:bg-white dark:hover:bg-slate-800 shadow-sm hover:shadow-md hover:-translate-y-0.5"}`}>
                                            <div className="absolute -left-6 sm:-left-8 top-1/2 w-6 sm:w-8 h-px bg-slate-200 dark:bg-slate-700/50 -z-10"></div>
                                            ...
                                            <div className="flex items-center gap-4 relative z-10 w-full">
                                                <div className={`w-10 h-10 flex items-center justify-center shrink-0 text-xl font-black italic opacity-30 ${isActive ? "text-white dark:text-slate-900" : "text-slate-400"}`}>
                                                    #{index + 1}
                                                </div>
                                                <div className="text-left flex-1 min-w-0 pr-2">
                                                    <span className={`block text-[10px] uppercase font-black tracking-widest mb-0.5 ${isActive ? "opacity-80 text-indigo-100 dark:text-indigo-900" : "text-slate-400"}`}>{isCompleted ? "Completed" : "In Progress"}</span>
                                                    <span className="block text-sm font-black leading-tight line-clamp-1">{p.title}</span>
                                                </div>
                                                <div className="shrink-0 flex items-center gap-3">
                                                    ...
                                                    <div className={`opacity-0 transform translate-x-[-10px] transition-all duration-300 ${isActive ? "opacity-100 translate-x-0" : "hidden sm:block sm:opacity-0"}`}>
                                                        <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M9 5l7 7-7 7" /></svg>
                                                    </div>
                                                </div>
                                            </div>
                            ...
                            {(!project.phases || project.phases.length === 0) && (
                                <div className="pl-8 ml-5 border-l-2 border-slate-200 dark:border-slate-700/50">
                                    <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-800/30 border border-slate-200 border-dashed text-center">
                                        <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">No Sub-phases defined</p>
                                    </div>
                                </div>
                            )}
                        ...
                        {showAllPhases && project.phases?.length > 3 && (
                            <button onClick={() => setShowAllPhases(false)} className="mt-2 text-xs font-black uppercase tracking-widest text-indigo-500 hover:text-indigo-600 flex items-center justify-center gap-1">Show Less <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 15l7-7 7 7" /></svg></button>
                        )}
                        ...
                        <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-2xl rounded-[2.5rem] md:rounded-[3rem] p-6 sm:p-8 border border-white/80 dark:border-slate-700/50 shadow-xl dark:shadow-[0_8px_30px_rgb(0,0,0,0.3)] relative overflow-hidden group">
                            ...
                            <form onSubmit={submitComment} className="flex flex-col gap-5 relative z-10">
                                ...
                                <AnimatePresence>
                                    {replyingTo && (
                                        <motion.div initial={{ opacity: 0, height: 0, y: -10 }} animate={{ opacity: 1, height: "auto", y: 0 }} exit={{ opacity: 0, height: 0, y: -10 }} className="flex items-center justify-between bg-indigo-50/80 dark:bg-indigo-500/10 border-l-4 border-indigo-500 text-indigo-700 dark:text-indigo-400 px-4 py-3 rounded-r-[1.25rem] rounded-l-sm text-[10px] sm:text-xs font-black uppercase tracking-widest shadow-sm overflow-hidden">
