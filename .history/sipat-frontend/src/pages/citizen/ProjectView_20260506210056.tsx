...
export default function ProjectView() {
    ...
    // 🚀 NEW: State to track who and what comment we are replying to
    ...
    const handleProjectReaction = async (type: 'likes' | 'neutrals' | 'unlikes') => {
        try {
            ...
            fetchProject(); // Refresh project data to show new score
    ...
    // 🚀 NEW: Activates the Reply State and scrolls to input
    ...
    const submitComment = async (e: React.FormEvent) => {
        ...
        try {
            ...
            await fetch("http://localhost:8080/api/comments", {
                ...
                body: JSON.stringify({
                    project_id: id,
                    parent_id: replyingTo?.id || null, // 🚀 NEW: Sends the parent ID!
        ...
        return (
            <div key={comment.id} className={`${isReply ? 'ml-8 md:ml-12 mt-4 pl-4 border-l-2 border-slate-200 dark:border-slate-800' : 'p-6 rounded-3xl border shadow-sm mt-6'} ${comment.is_ghost_alert ? 'bg-red-50 dark:bg-red-500/10 border-red-200' : comment.ai_match_score >= 40 ? 'bg-emerald-50 dark:bg-emerald-500/10 border-emerald-200' : 'bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800'}`}>
                ...
                <div className="flex flex-wrap items-center gap-6 border-t border-slate-200/60 dark:border-slate-800/60 pt-4 mt-2">
                    ...
                    <button onClick={() => handleReaction(comment.id, 'neutrals')} className="text-xs font-bold text-slate-500 hover:text-slate-900 transition-colors">😐 ({comment.neutrals || 0})</button>
                    ...
                    {/* Hide reply button on deep replies to prevent extreme nesting */}
                    {!isReply && (
        ...
        <div className="max-w-5xl mx-auto py-10 px-4 flex flex-col md:flex-row gap-10">
            ...
            <div className="w-full md:w-1/3 flex flex-col gap-6">
                <div>
                    ...
                    <div className="flex flex-wrap gap-4 mb-5">
                        ...
                        <div className="bg-slate-50 dark:bg-slate-800/50 px-3 py-1.5 rounded-lg border border-slate-200">
                            ...
                            <span className="text-sm font-bold text-slate-700">{formatDate(project.start_date)} - {formatDate(project.completion_date)}</span>
                        </div>
                    </div>
                    ...
                    <p className="text-slate-500 dark:text-slate-400 text-sm leading-relaxed mt-4">{project.description}</p>
                ...
                <div className="bg-slate-50 dark:bg-slate-900/50 rounded-2xl p-2 flex flex-col gap-1 border border-slate-200 mt-2">
                    ...
                    <button onClick={() => setActiveTab("Overall Project")} className={`text-left px-4 py-3 rounded-xl text-sm font-bold transition-all ${activeTab === "Overall Project" ? "bg-white shadow-sm text-indigo-600" : "text-slate-500 hover:bg-slate-100"}`}>Overall Project</button>
                    {project.phases?.map((p: any) => (
                        <button key={p.title} onClick={() => setActiveTab(p.title)} className={`text-left flex items-center justify-between px-4 py-3 rounded-xl text-sm font-bold transition-all ${activeTab === p.title ? "bg-white shadow-sm text-indigo-600" : "text-slate-500 hover:bg-slate-100"}`}>
                            <span className="truncate pr-2">{p.title}</span>
                            {(p.status === "completed" || p.image_url) && <svg className="w-4 h-4 text-emerald-500 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" /></svg>}
                        </button>
                    ))}
                </div>
            </div>
            ...
            <div className="w-full md:w-2/3 flex flex-col gap-6">
                ...
                <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-200 shadow-sm">
                    ...
                    <form onSubmit={submitComment} className="flex flex-col gap-4">
                        ...
                        {replyingTo && (
                            <div className="flex items-center justify-between bg-indigo-50 dark:bg-indigo-500/10 text-indigo-700 dark:text-indigo-300 px-3 py-2 rounded-lg text-xs font-bold border border-indigo-100 dark:border-indigo-500/30">
