...
interface Project {
  ...
  neutrals?: number;
  unlikes?: number;
  organization_name?: string; // 🚀 Added to interface
}
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {projects.map((project, idx) => {
            ...
            // 🚀 Defaulting to DPWH if backend doesn't provide organization_name yet
            const orgName = project.organization_name || "DPWH";

            return (
              <motion.div key={project.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.05 }} className="bg-white dark:bg-slate-900 rounded-[2rem] border border-slate-200/60 dark:border-slate-800/60 overflow-hidden shadow-sm hover:shadow-md transition-shadow flex flex-col relative">

                <button onClick={() => toggleSave(project.id)} className="absolute top-4 right-4 z-10 bg-white/90 dark:bg-slate-900/90 backdrop-blur-md p-2.5 rounded-full shadow-md hover:scale-110 transition-transform">
                ...
                <div className="p-6 flex flex-col flex-grow gap-4">
                  <div>
                    <h3 className="text-xl font-bold text-slate-900 dark:text-white line-clamp-2 leading-tight mb-2">{project.title}</h3>

                    {/* 🚀 NEW: Agency Profile Link */}
                    <Link to={`/agency-profile/${encodeURIComponent(orgName)}`} className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 rounded-lg text-[10px] font-black uppercase tracking-widest hover:bg-indigo-100 dark:hover:bg-indigo-500/20 transition-colors w-fit">
                      🏛️ {orgName}
                    </Link>
                  </div>
                  ...
                  <div className="flex items-center justify-between mt-2 pt-4 border-t border-slate-100 dark:border-slate-800">
                    ...
                    <div className="flex items-center gap-3">
                      <motion.button whileHover={{ scale: 1.15 }} whileTap={{ scale: 0.8 }} onClick={() => handleReaction(project.id, 'likes')} className="flex items-center gap-1 text-xs font-bold text-slate-500 hover:text-emerald-600 transition-colors">
                        👍 <span>{project.likes || 0}</span>
                      </motion.button>
                      <motion.button whileHover={{ scale: 1.15 }} whileTap={{ scale: 0.8 }} onClick={() => handleReaction(project.id, 'neutrals')} className="flex items-center gap-1 text-xs font-bold text-slate-500 hover:text-slate-900 dark:hover:text-white transition-colors">
                        😐 <span>{project.neutrals || 0}</span>
                      </motion.button>
                      <motion.button whileHover={{ scale: 1.15 }} whileTap={{ scale: 0.8 }} onClick={() => handleReaction(project.id, 'unlikes')} className="flex items-center gap-1 text-xs font-bold text-slate-500 hover:text-red-600 transition-colors">
