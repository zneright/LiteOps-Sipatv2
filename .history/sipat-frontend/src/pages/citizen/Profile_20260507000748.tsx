...
export default function Profile() {
  ...
  // 🚀 DYNAMIC ANIMATIONS BASED ON ENGAGEMENT LEVEL
  const getBadgeAnimation = (count: number) => {
    if (count >= 250) return { // Level 5: Epic 3D Spin & Scale
      animate: { rotateY: [0, 360], scale: [1, 1.2, 1] },
      transition: { duration: 2, repeat: Infinity, ease: "linear" }
    };
    if (count >= 50) return { // Level 4: Float + Tilt + Scale
      animate: { y: [0, -15, 0], rotate: [0, 10, -10, 0], scale: [1, 1.1, 1] },
      transition: { duration: 2.5, repeat: Infinity, ease: "easeInOut" }
    };
    if (count >= 10) return { // Level 3: High Float
      animate: { y: [0, -15, 0] },
      transition: { duration: 2, repeat: Infinity, ease: "easeInOut" }
    };
    if (count >= 1) return { // Level 2: Gentle Float
      animate: { y: [0, -5, 0] },
      transition: { duration: 3, repeat: Infinity, ease: "easeInOut" }
    };
    // Level 1: Just a slow pulse for newcomers
    return {
  ...
  useEffect(() => {
    const fetchEverything = async () => {
      ...
      try {
        ...
        // Filter engagements by current user
        // This is much safer than checking by name!
  ...
  // This calculates the percentage ONLY within the current tier level!
  const progressPercent = currentCount >= 1000
    ? 100
    : ((currentCount - details.prevGoal) / (details.nextGoal - details.prevGoal)) * 100;
  ...
  return (
    <div className="w-full flex flex-col gap-6 pb-20">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] p-8 border border-slate-200 dark:border-slate-800 shadow-sm relative overflow-hidden">
        ...
        {/* Giant Background Badge with escalating animation */}
        <motion.div
          className="absolute -top-10 -right-10 p-10 opacity-5 dark:opacity-10 text-[12rem] pointer-events-none"
        ...
        <div className="flex flex-col md:flex-row gap-6 items-center relative z-10">
          ...
          <div className="flex-1 text-center md:text-left">
            ...
            <div className="flex items-center justify-center md:justify-start gap-2 mt-1">
              <motion.span
                className={`text-xl inline-block origin-center ${details.badgeColor}`}
                animate={badgeAnim.animate}
                transition={badgeAnim.transition}
              >
                {details.badge}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        {/* Progress Card */}
        <div className="bg-white dark:bg-slate-900 p-6 rounded-[2rem] border border-slate-200 dark:border-slate-800 shadow-sm col-span-1 md:col-span-2 flex flex-col justify-center overflow-hidden relative">
          ...
          <div className="w-full h-4 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden relative z-10">
            <motion.div
              ...
              className={`h-full bg-current ${details.badgeColor} relative`}
            >
              {/* Add a shimmer effect to the progress bar if they have high engagement */}
              {currentCount >= 25 && (
                <motion.div
