  ...
  useEffect(() => {
    const fetchEverything = async () => {
      ...
      try {
        ...
        // Fetch All Comments & Replies across projects
        ...
        // Filter for this user's engagements (Top-level comments + Replies)
        const filtered = results.flat().filter((c: any) => c.user_email === user?.email);
        setMyComments(filtered);

        // 🚀 ADVANCED TRUST SCORE MATH
        const totalEngagements = filtered.length;
        const ghostCount = filtered.filter(c => Number(c.is_ghost_alert) === 1).length;

        // Calculate total community feedback
        const totalLikes = filtered.reduce((sum, curr) => sum + Number(curr.likes || 0), 0);
        const totalUnlikes = filtered.reduce((sum, curr) => sum + Number(curr.unlikes || 0), 0);

        // Base Score from AI Accuracy
        const avgAiScore = totalEngagements > 0
          ? filtered.reduce((acc, curr) => acc + Number(curr.ai_match_score || 0), 0) / totalEngagements
          : 100;

        // Final Score: AI Average + (Likes * 2) - (Unlikes * 3) - (Ghosts * 20)
        const communityBonus = (totalLikes * 2) - (totalUnlikes * 3);
        const ghostPenalty = ghostCount * 20;

        const finalTrust = Math.max(0, Math.min(100, Math.round(avgAiScore + communityBonus - ghostPenalty)));
  ...
  return (
    <div className="w-full flex flex-col gap-8 pb-20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
        ...
        {/* Progress Card (Spans 5 columns) */}
        <div className="col-span-1 md:col-span-5 bg-white dark:bg-slate-900 p-6 rounded-[2rem] border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col justify-center overflow-hidden relative">
          ...
          <p className="text-xs font-bold text-slate-400 mt-3 text-right relative z-10">
            {currentCount >= 1000 ? "Max Rank Reached!" : `${details.nextGoal - currentCount} more needed to rank up`}
          </p>
        </div>

        {/* Community Upvotes Card (Spans 3 columns) */}
        <div className="col-span-1 md:col-span-3 bg-white dark:bg-slate-900 p-6 rounded-[2rem] border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col justify-center text-center items-center">
          <div className="text-4xl mb-3">👍</div>
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Community Likes</p>
          <p className="text-3xl font-black mt-1 text-blue-500">{details.totalLikes}</p>
          <p className="text-xs font-bold text-slate-500 mt-2">Earned from peers</p>
        </div>

        {/* Deep Dive AI & Community Trust Card (Spans 4 columns) */}
        <div className="col-span-1 md:col-span-4 bg-white dark:bg-slate-900 p-6 rounded-[2rem] border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col justify-between">
          <div>
            <div className="flex justify-between items-start">
              ...
              <span className={`px-2 py-1 text-[9px] font-black uppercase rounded-md ${details.trustScore >= 80 ? 'bg-emerald-100 text-emerald-700' : details.trustScore >= 50 ? 'bg-amber-100 text-amber-700' : 'bg-red-100 text-red-700'}`}>
                {details.trustScore >= 80 ? 'Trusted Auditor' : details.trustScore >= 50 ? 'Developing' : 'Critical'}
              </span>
            </div>
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Combined Trust Score</p>
          ...
          <div className="mt-4 pt-4 border-t border-slate-100 dark:border-slate-800">
            <p className="text-xs font-bold text-slate-500 mb-2">Score Breakdown:</p>
            <ul className="text-[10px] text-slate-400 space-y-1">
              <li>🤖 AI Fact-Check Base Score</li>
              <li>👍 Community Likes (+ points)</li>
              <li>{details.ghostCount > 0 ? `⚠️ -${details.ghostCount * 20}% from Ghost Alerts.` : `✅ Zero Ghost Alert Penalties.`}</li>
            </ul>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] p-8 border border-slate-200 dark:border-slate-800 shadow-sm">
        ...
        {isLoading ? (
          ...
          <div className="text-center py-10 bg-slate-50 dark:bg-slate-800/50 rounded-[2rem] border-2 border-dashed border-slate-200 dark:border-slate-700">
            <p className="text-sm font-bold text-slate-500 mb-2">No active engagements yet.</p>
            <p className="text-xs text-slate-400 mb-4">Start verifying or replying to projects to build your Trust Score.</p>
          </div>
        ) : (
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">
                      {comment.parent_id ? "↳ Replied in Thread" : (comment.target_phase || "Overall Review")}
                    </p>
                    {Number(comment.likes) > 0 && <span className="text-[10px] bg-blue-100 text-blue-600 font-black px-1.5 rounded">+{comment.likes} Likes</span>}
                  </div>
