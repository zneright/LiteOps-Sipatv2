...
export default function Profile() {
  ...
  if (isLoading) {
    return (
      <div className="w-full min-h-screen flex flex-col justify-center items-center gap-6 relative overflow-hidden bg-slate-50 dark:bg-slate-950">
        <div className="absolute inset-0 pointer-events-none -z-10 flex items-center justify-center">
          <motion.div animate={{ scale: [1, 1.2, 1], opacity: [0.1, 0.3, 0.1] }} transition={{ repeat: Infinity, duration: 2 }} className="w-64 h-64 bg-indigo-500 blur-[80px] rounded-full mix-blend-multiply dark:mix-blend-screen" />
        </div>
        <div className="relative">
          <motion.div animate={{ rotate: 360 }} transition={{ duration: 1.5, repeat: Infinity, ease: "linear" }} className="absolute inset-0 rounded-full border-t-4 border-indigo-500 opacity-40"></motion.div>
          <motion.div animate={{ rotate: -360 }} transition={{ duration: 2, repeat: Infinity, ease: "linear" }} className="h-16 w-16 md:h-20 md:w-20 rounded-full border-l-4 border-purple-500 shadow-[0_0_20px_rgba(99,102,241,0.3)] flex items-center justify-center text-2xl md:text-3xl bg-white/50 dark:bg-slate-900/50 backdrop-blur-md">🔍</motion.div>
        </div>
        <motion.p animate={{ opacity: [0.5, 1, 0.5] }} transition={{ repeat: Infinity, duration: 1.5 }} className="text-xs md:text-sm font-black uppercase tracking-[0.2em] text-indigo-600 dark:text-indigo-400">Syncing Identity...</motion.p>
      </div>
    );
  }

  return (
    <div className="w-full min-h-screen flex flex-col relative overflow-x-hidden font-sans pb-24 bg-[#F8FAFC] dark:bg-[#0B1120] selection:bg-indigo-500/30">
      ...
      <div className="max-w-[85rem] mx-auto w-full px-4 sm:px-6 lg:px-8 py-8 md:py-12 flex flex-col gap-8 md:gap-12 relative z-10">
        ...
        <motion.div initial={{ opacity: 0, y: 30, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ duration: 0.6, type: "spring", bounce: 0.4 }} className="relative bg-white/70 dark:bg-slate-900/60 backdrop-blur-2xl rounded-[2.5rem] md:rounded-[3rem] border border-white/80 dark:border-slate-700/50 shadow-xl dark:shadow-[0_8px_30px_rgb(0,0,0,0.3)] overflow-hidden">
          ...
          <div className="absolute top-0 left-0 w-full h-40 sm:h-56 bg-slate-100 dark:bg-slate-950 overflow-hidden">
            <motion.div animate={{ x: [0, 40, 0], y: [0, -20, 0] }} transition={{ duration: 15, repeat: Infinity, ease: "easeInOut" }} className={`absolute -top-20 -left-20 w-64 h-64 md:w-80 md:h-80 bg-gradient-to-br ${activeRank.grad} rounded-full blur-[60px] md:blur-[80px] opacity-40 md:opacity-30`} />
            <motion.div animate={{ x: [0, -40, 0], y: [0, 30, 0] }} transition={{ duration: 18, repeat: Infinity, ease: "easeInOut" }} className="absolute top-10 -right-20 w-72 h-72 md:w-96 md:h-96 bg-indigo-500 rounded-full blur-[80px] md:blur-[100px] opacity-30 md:opacity-20" />
          ...
          <div className="px-6 sm:px-10 pb-8 sm:pb-10 pt-24 sm:pt-36 relative z-10 flex flex-col sm:flex-row items-center sm:items-end gap-6 md:gap-8 text-center sm:text-left">
            <div className="relative group">
              <div className="w-32 h-32 sm:w-40 sm:h-40 rounded-[2rem] sm:rounded-[2.5rem] bg-white/60 dark:bg-slate-800/60 p-2 backdrop-blur-xl shadow-2xl shrink-0 -mt-24 sm:-mt-16 relative z-10 transition-transform duration-500 group-hover:scale-105 border border-white/50 dark:border-slate-700/50">
            ...
            <div className="flex-1 w-full flex flex-col items-center sm:items-start gap-2">
              <h1 className="text-4xl sm:text-5xl font-black text-slate-900 dark:text-white tracking-tighter leading-none drop-shadow-sm">{user?.name || "Citizen"}</h1>
              <div className="flex flex-wrap justify-center sm:justify-start items-center gap-3 mt-1 sm:mt-2">
                <span className={`px-4 py-1.5 rounded-full text-[10px] md:text-[11px] font-black uppercase tracking-[0.15em] ${activeRank.bg} ${activeRank.color} shadow-sm border border-white/40 dark:border-slate-700/50 flex items-center gap-1.5`}>
            ...
            <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => setIsEditing(true)} className="px-6 md:px-8 py-3 md:py-3.5 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-[1.25rem] font-black text-xs md:text-sm uppercase tracking-widest shadow-xl hover:shadow-[0_10px_30px_rgba(0,0,0,0.2)] dark:hover:shadow-[0_10px_30px_rgba(255,255,255,0.2)] transition-all duration-300 w-full sm:w-auto mt-4 sm:mt-0 flex items-center justify-center gap-2 border border-slate-700 dark:border-slate-200">
        ...
        <motion.div variants={staggerContainer} initial="hidden" animate="show" className="w-full flex flex-col gap-3">
          <div className="flex items-center justify-between px-2">
            ...
            <span className="text-[10px] md:text-xs font-black bg-white/60 dark:bg-slate-800/60 backdrop-blur-md px-3 py-1.5 rounded-full text-slate-600 dark:text-slate-300 border border-slate-200/50 dark:border-slate-700/50 shadow-sm">{ALL_RANKS.filter(r => currentCount >= r.req).length} / {ALL_RANKS.length} Unlocked</span>
          </div>

          <div className="relative">
            <div className="absolute left-0 top-0 bottom-0 w-12 bg-gradient-to-r from-[#F8FAFC] dark:from-[#0B1120] to-transparent z-10 pointer-events-none sm:hidden"></div>
            <div className="absolute right-0 top-0 bottom-0 w-12 bg-gradient-to-l from-[#F8FAFC] dark:from-[#0B1120] to-transparent z-10 pointer-events-none sm:hidden"></div>

            <div className="flex gap-3 sm:gap-5 overflow-x-auto pb-6 pt-2 scrollbar-hide snap-x px-2">
              {ALL_RANKS.map((rank) => {
                const isUnlocked = currentCount >= rank.req;
                return (
                  <motion.div key={rank.id} variants={fadeUp} whileHover={isUnlocked ? { y: -8, scale: 1.05 } : {}} className={`shrink-0 snap-center relative flex flex-col items-center justify-center p-4 rounded-[1.5rem] md:rounded-[2rem] w-24 h-24 sm:w-[110px] sm:h-[110px] transition-all duration-300 ${isUnlocked ? `bg-white/70 dark:bg-slate-900/70 backdrop-blur-xl border-2 border-white/80 dark:border-slate-700/50 shadow-lg hover:shadow-xl cursor-help` : 'bg-slate-200/50 dark:bg-slate-800/30 border border-slate-300/50 dark:border-slate-800/50 opacity-60 grayscale'}`}>
                    {isUnlocked && (
                      <div className={`absolute inset-0 bg-gradient-to-br ${rank.grad} opacity-[0.08] dark:opacity-10 rounded-[1.5rem] md:rounded-[2rem] pointer-events-none`} />
                    )}
                    <div className="text-3xl sm:text-4xl mb-2 relative z-10 drop-shadow-md">{rank.icon}</div>
                    ...
                    {!isUnlocked && (
                      <div className="absolute inset-0 bg-slate-900/5 dark:bg-slate-900/40 rounded-[1.5rem] md:rounded-[2rem] flex items-center justify-center backdrop-blur-[1px]">
        ...
        <motion.div variants={staggerContainer} initial="hidden" animate="show" className="bg-white/70 dark:bg-slate-900/60 backdrop-blur-2xl rounded-[2.5rem] md:rounded-[3rem] p-6 sm:p-10 border border-white/80 dark:border-slate-700/50 shadow-xl dark:shadow-[0_8px_30px_rgb(0,0,0,0.2)]">
          ...
          {myComments.length === 0 ? (
            <div className="bg-slate-50/60 dark:bg-slate-800/40 p-10 sm:p-16 rounded-[2rem] md:rounded-[2.5rem] border-2 border-dashed border-slate-200 dark:border-slate-700 flex flex-col items-center justify-center text-center shadow-sm">
              ...
              <p className="text-slate-500 dark:text-slate-400 font-medium mb-8 text-sm max-w-sm leading-relaxed">Your audit history is empty. Start reviewing projects to build your track record and level up.</p>
              <Link to="/explore" className="px-8 py-3.5 bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 font-black text-xs uppercase tracking-widest rounded-2xl hover:bg-indigo-100 dark:hover:bg-indigo-500/20 transition-colors border border-indigo-100 dark:border-indigo-500/20 flex items-center gap-2 shadow-sm transform hover:-translate-y-0.5 active:translate-y-0">
              ...
              {myComments.slice().reverse().map((comment, idx) => (
                <motion.div key={comment.id} variants={fadeUp} className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-md p-5 sm:p-6 rounded-[1.5rem] md:rounded-[2rem] border border-slate-100 dark:border-slate-700/60 shadow-[0_4px_20px_rgb(0,0,0,0.03)] dark:shadow-[0_4px_20px_rgb(0,0,0,0.2)] flex flex-col gap-4 group hover:shadow-lg hover:border-indigo-100 dark:hover:border-indigo-500/30 transition-all duration-300 transform-gpu hover:-translate-y-1">
                  ...
                  <div className="bg-slate-50/60 dark:bg-slate-900/60 p-5 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-inner">
                    <p className="text-[14px] md:text-[15px] text-slate-700 dark:text-slate-300 font-medium leading-relaxed italic">"{comment.text_content}"</p>
                  </div>
          ...
          {isEditing && (
            <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-0 sm:p-4 bg-slate-900/40 dark:bg-black/60 backdrop-blur-md">
              ...
              >
                ...
                <h2 className="text-2xl md:text-3xl font-black mb-2 text-slate-900 dark:text-white uppercase tracking-tight text-center">Edit Identity</h2>
                ...
                <div className="flex flex-col gap-6 flex-1 overflow-y-auto scrollbar-hide pb-10 sm:pb-0 px-1">
                  <div>
                    ...
                    <input type="text" placeholder="e.g. Obando, Bulacan" value={details.location} onChange={(e) => setDetails({ ...details, location: e.target.value })} className="w-full p-4 rounded-2xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-[15px] focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-all text-slate-900 dark:text-white shadow-inner font-medium placeholder:text-slate-400" />
                  </div>
                  <div>
                    <label className="text-[10px] sm:text-xs font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest ml-2 mb-2 flex items-center gap-1.5">
                      <svg className="w-3.5 h-3.5 text-purple-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>
                      Professional Bio
                    </label>
                    <textarea placeholder="Your advocacy, profession, or passion..." value={details.bio} onChange={(e) => setDetails({ ...details, bio: e.target.value })} className="w-full p-4 rounded-2xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-[15px] focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-all resize-none text-slate-900 dark:text-white h-32 shadow-inner font-medium placeholder:text-slate-400 leading-relaxed" rows={4} />
                  </div>
                  <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 mt-auto sm:mt-4 pt-4 border-t border-slate-100 dark:border-slate-800">
                    <button onClick={() => setIsEditing(false)} disabled={isSaving} className="w-full py-4 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 rounded-[1.25rem] font-black text-[11px] uppercase tracking-widest disabled:opacity-50 transition-colors hover:bg-slate-200 dark:hover:bg-slate-700 shadow-sm border border-slate-200 dark:border-slate-700">Cancel</button>
                    <button onClick={handleSaveProfile} disabled={isSaving} className="w-full py-4 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-[1.25rem] font-black text-[11px] uppercase tracking-widest shadow-xl shadow-slate-900/20 dark:shadow-white/20 disabled:opacity-50 transition-transform active:scale-95 flex justify-center items-center gap-2">
                      {isSaving ? (
                        <>
