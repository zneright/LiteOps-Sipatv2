            ...
            {ALL_RANKS.map((rank) => {
              ...
              return (
                <motion.div key={rank.id} variants={fadeUp} whileHover={isUnlocked ? { y: -8, scale: 1.05 } : {}} className={`relative flex flex-col items-center justify-center p-3 sm:p-4 rounded-[1.25rem] sm:rounded-[1.5rem] w-full aspect-square transition-all duration-300 ${isUnlocked ? `bg-white/70 dark:bg-slate-900/70 backdrop-blur-md border border-white/80 dark:border-slate-700/50 shadow-md hover:shadow-xl cursor-help` : 'bg-slate-200/50 dark:bg-slate-800/30 border border-slate-300/50 dark:border-slate-800/50 opacity-60 grayscale'}`}>
                  ...
                  {!isUnlocked && (
                    <div className="absolute inset-0 bg-slate-900/10 dark:bg-slate-900/40 rounded-[1.25rem] sm:rounded-[1.5rem] flex items-center justify-center backdrop-blur-[1px]">
