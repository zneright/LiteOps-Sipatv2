...
export default function Explore() {
  ...
  useEffect(() => {
    const fetchProjects = async () => {
      try {
        ...
        if (selectedCategory) {
          // 🚀 FIX: Added .toLowerCase() and .trim() so "infrastructure " matches "Infrastructure" safely
  ...
  // 🚀 FIX: Bulletproof JSON Parser. Always forces an Array return to prevent .filter() crashes
  ...
  const getProgressData = (phasesData: any) => {
    ...
    // 🚀 FIX: Ensure image_url actually contains a string, preventing 'undefined' from counting as completed
            ...
            return (
              ...
              >
                <div className="h-40 bg-slate-100 dark:bg-slate-800 relative overflow-hidden">
                  {/* 🚀 FIX: Added typeof check to prevent crashes if file_url is strangely formatted */}
                  {project.file_url ? (
                ...
                <div className="p-6 flex flex-col flex-grow gap-4">
                  ...
                  <div className="flex items-center justify-between mt-2 pt-4 border-t border-slate-100 dark:border-slate-800">
                    ...
                    <Link to={`/project/${project.id}`} className="text-sm font-bold text-indigo-600 dark:text-indigo-400 hover:text-indigo-700 transition-colors">View Details</Link>
                  </div>
                </div>
              </motion.div>
            );
