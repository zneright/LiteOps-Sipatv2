    ...
    const renderComment = (comment: any, isReply = false, parentName = "") => {
        ...
        return (
            <div key={comment.id} className="relative z-10">
                ...
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className={` ${isReply ? 'ml-6 sm:ml-8 mt-4' : 'mt-6'} p-5 sm:p-6 rounded-[1.5rem] sm:rounded-[2rem] border transition-all duration-300 shadow-sm hover:shadow-md relative z-10 ${isGhost ? 'bg-rose-50/80 dark:bg-rose-900/10 border-rose-200/60 dark:border-rose-500/20' : comment.ai_match_score >= 40 ? 'bg-emerald-50/80 dark:bg-emerald-900/10 border-emerald-200/60 dark:border-emerald-500/20' : 'bg-white/80 dark:bg-slate-800/60 border-slate-100 dark:border-slate-700/50 backdrop-blur-xl'} `}>
                    ...
                    <div className="flex flex-wrap items-center justify-between border-t border-slate-200/60 dark:border-slate-700/50 pt-4 mt-2 gap-4">
                        <div className="flex items-center gap-1.5 bg-slate-50 dark:bg-slate-800/50 p-1.5 rounded-[1rem] border border-slate-100 dark:border-slate-700/50 shadow-sm w-full sm:w-auto justify-between sm:justify-start">
                            <motion.button whileHover={{ scale: 1.15, rotate: -10 }} whileTap={{ scale: 0.9 }} onClick={() => handleReaction(comment.id, 'likes')} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg hover:bg-white dark:hover:bg-slate-700 text-xs font-black text-slate-500 dark:text-slate-400 hover:text-emerald-600 dark:hover:text-emerald-400 transition-all shadow-sm">👍 <span>{comment.likes || 0}</span></motion.button>
                            <div className="w-px h-3 bg-slate-200 dark:bg-slate-700"></div>
                            <motion.button whileHover={{ scale: 1.15 }} whileTap={{ scale: 0.9 }} onClick={() => handleReaction(comment.id, 'neutrals')} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg hover:bg-white dark:hover:bg-slate-700 text-xs font-black text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition-all shadow-sm">😐 <span>{comment.neutrals || 0}</span></motion.button>
                            <div className="w-px h-3 bg-slate-200 dark:bg-slate-700"></div>
                            <motion.button whileHover={{ scale: 1.15, rotate: 10 }} whileTap={{ scale: 0.9 }} onClick={() => handleReaction(comment.id, 'unlikes')} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg hover:bg-white dark:hover:bg-slate-700 text-xs font-black text-slate-500 dark:text-slate-400 hover:text-rose-600 dark:hover:text-rose-400 transition-all shadow-sm">👎 <span>{comment.unlikes || 0}</span></motion.button>
                        </div>
            ...
            <div className="relative max-w-[90rem] mx-auto py-8 sm:py-12 px-4 sm:px-6 lg:px-8 flex flex-col xl:flex-row gap-8 xl:gap-12">
                ...
                <div className="w-full xl:w-[55%] flex flex-col gap-8">
                    <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ type: "spring", bounce: 0.4 }} className="flex flex-col gap-8">
                        ...
                        {/* Enhanced Phases Tabs (Pill Style) */}
                        <div className="bg-white/60 dark:bg-slate-900/60 backdrop-blur-2xl rounded-3xl p-3 flex flex-row overflow-x-auto hide-scrollbar gap-2 border border-white/80 dark:border-slate-700/50 shadow-lg relative z-20">
                            <button
                                onClick={() => setActiveTab("Overall Project")}
                                className={`shrink-0 flex items-center justify-center px-6 py-3 rounded-2xl text-[11px] sm:text-xs font-black uppercase tracking-widest transition-all duration-300 relative ${activeTab === "Overall Project" ? "text-white shadow-[0_4px_15px_rgba(99,102,241,0.4)] transform scale-[1.02]" : "text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-white/50 dark:hover:bg-slate-800/50"}`}
                            >
                                {activeTab === "Overall Project" && (
                                    <motion.div layoutId="phasePill" className="absolute inset-0 bg-gradient-to-r from-indigo-500 to-purple-600 rounded-2xl -z-10" transition={{ type: "spring", bounce: 0.2, duration: 0.6 }} />
                                )}
                            ...
                            {project.phases?.map((p: any, index: number) => {
                                ...
                                return (
                                    <button
                                        ...
                                        className={`shrink-0 flex items-center justify-center px-6 py-3 rounded-2xl text-[11px] sm:text-xs font-black uppercase tracking-widest transition-all duration-300 relative ${isActive ? "text-white shadow-[0_4px_15px_rgba(99,102,241,0.4)] transform scale-[1.02]" : "text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-white/50 dark:hover:bg-slate-800/50"}`}
                                    >
                                        {isActive && (
                                            <motion.div layoutId="phasePill" className="absolute inset-0 bg-gradient-to-r from-indigo-500 to-purple-600 rounded-2xl -z-10" transition={{ type: "spring", bounce: 0.2, duration: 0.6 }} />
                                        )}
                                        <span className="relative z-10 flex items-center gap-2">
                                            {isCompleted && <div className="w-1.5 h-1.5 rounded-full bg-emerald-400 shadow-[0_0_8px_rgba(52,211,153,0.8)]"></div>}
                                            <span className="truncate max-w-[150px]">Phase {index + 1}</span>
                                        </span>
                                    </button>
                                );
                            })}
