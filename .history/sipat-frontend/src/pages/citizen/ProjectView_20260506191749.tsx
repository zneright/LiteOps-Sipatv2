...
import { useAuth } from "../../context/AuthContext"; // 🚀 Added useAuth to get user name
...
export default function ProjectView() {
    ...
    // UI State - 🚀 Defaults to "Overall Project" first
    ...
    // 🚀 NEW: Added missing handleReaction function so your like/unlike buttons work!
    const handleReaction = async (commentId: number, type: 'likes' | 'neutrals' | 'unlikes') => {
        try {
            ...
            fetchComments(); // Refresh comments to show new counts
        } catch (error) {
    ...
    const submitComment = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // 🚀 FIX: Automatically use the logged-in user's name context
            ...
            await fetch("http://localhost:8080/api/comments", {
                ...
                body: JSON.stringify({
                    ...
                    author_name: finalAuthorName, // 🚀 Now uses context!
    ...
    return (
        <div className="max-w-5xl mx-auto py-10 px-4 flex flex-col md:flex-row gap-10">
            ...
            <div className="w-full md:w-1/3 flex flex-col gap-6">
                <div>
                    {/* 🚀 FIX: Added break-words to prevent long titles from overlapping */}
                    <h1 className="text-3xl font-black mb-2 break-words leading-tight">{project.title}</h1>
                ...
                <div className="bg-slate-50 rounded-2xl p-2 flex flex-col gap-1 border border-slate-200">
                    ...
                    {/* 🚀 FIX: Renamed to "Overall Project" */}
                    <button
                        onClick={() => setActiveTab("Overall Project")}
                        className={`text-left px-4 py-3 rounded-xl text-sm font-bold transition-all ${activeTab === "Overall Project" ? "bg-white shadow-sm text-indigo-600" : "text-slate-500 hover:bg-slate-100"}`}
                    >
                        Overall Project
                    </button>
                ...
                <div className="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm">
                    ...
                    <form onSubmit={submitComment} className="flex flex-col gap-4">
                        <div className="flex items-center gap-4">
                            ...
                            {/* 🚀 FIX: Removed text input, now automatically displays their name based on login state */}
                            {!isAnonymous && (
