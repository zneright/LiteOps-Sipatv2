...
export default function PublicAgencyProfile() {
    ...
    if (!details) return (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-5xl mx-auto py-20 px-4 text-center">
            ...
            <button onClick={() => navigate(-1)} className="px-6 py-2 bg-indigo-600 hover:bg-indigo-700 dark:bg-indigo-500 dark:hover:bg-indigo-600 text-white rounded-xl font-bold transition-colors">Go Back</button>
        </motion.div>
    );

    return (
        <div className="w-full max-w-5xl mx-auto flex flex-col gap-8 py-8 px-4 sm:px-6 pb-20">
            <button onClick={() => navigate(-1)} className="inline-flex items-center gap-2 text-sm font-bold text-slate-500 dark:text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors w-fit">
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M10 19l-7-7m0 0l7-7m-7 7h18" /></svg>
                Back to Feed
            </button>

            <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, ease: "easeOut" }} className="relative bg-white dark:bg-slate-800/90 dark:backdrop-blur-xl rounded-[2.5rem] shadow-xl dark:shadow-2xl dark:shadow-slate-900/50 border border-slate-200/60 dark:border-slate-700/60 overflow-hidden">
                ...
                <div className="relative z-10 pt-20 px-6 sm:px-10 pb-8 flex flex-col md:flex-row gap-6 md:gap-8 items-center md:items-end">
                    <div className="relative shrink-0">
                        <div className="w-32 h-32 rounded-[2rem] bg-white dark:bg-slate-800 border-4 border-white dark:border-slate-800 flex items-center justify-center text-6xl font-black text-slate-800 dark:text-white shadow-2xl overflow-hidden">
                    ...
                    <div className="flex-1 text-center md:text-left flex flex-col items-center md:items-start gap-1 pb-1">
                        <div className="flex flex-col sm:flex-row items-center gap-4">
                            <h1 className="text-3xl sm:text-4xl font-black text-slate-900 dark:text-white uppercase tracking-tight">{details.organization_name}</h1>
                            <button onClick={handleToggleFollow} className={`px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest transition-all shadow-sm flex items-center gap-1.5 ${isFollowing ? "bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-300 hover:bg-rose-50 dark:hover:bg-rose-500/10 hover:text-rose-500 dark:hover:text-rose-400 border border-slate-200 dark:border-slate-600" : "bg-indigo-600 dark:bg-indigo-500 text-white hover:bg-indigo-700 dark:hover:bg-indigo-600 hover:scale-105 active:scale-95"}`}>
                ...
                <div className="px-6 sm:px-10 pb-10 border-t border-slate-100 dark:border-slate-700/50 pt-6 mt-2">
                    ...
                    <p className="text-slate-700 dark:text-slate-300 text-[15px] leading-relaxed max-w-4xl border-l-4 border-indigo-500 dark:border-indigo-400 pl-4 py-1">{details.bio}</p>
                </div>
            </motion.div>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1, duration: 0.5 }} whileHover={{ y: -5 }} className="bg-white dark:bg-slate-800 p-8 rounded-[2rem] border border-slate-200 dark:border-slate-700/60 shadow-sm dark:shadow-none flex flex-col items-center justify-center text-center relative overflow-hidden group hover:shadow-xl dark:hover:shadow-indigo-500/10 transition-all">
                    <div className="absolute -top-10 -right-10 w-40 h-40 bg-indigo-500 opacity-[0.03] group-hover:opacity-[0.08] dark:opacity-[0.05] dark:group-hover:opacity-[0.1] rounded-full blur-3xl transition-opacity duration-500" />
                    ...
                    <div className="relative">
                        <svg className="w-32 h-32 transform -rotate-90">
                            <circle cx="64" cy="64" r="56" fill="transparent" stroke="currentColor" strokeWidth="8" className="text-slate-100 dark:text-slate-700/50" />
                ...
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2, duration: 0.5 }} whileHover={{ y: -5 }} className="bg-white dark:bg-slate-800 p-8 rounded-[2rem] border border-slate-200 dark:border-slate-700/60 shadow-sm dark:shadow-none flex flex-col items-center justify-center text-center group hover:shadow-xl dark:hover:shadow-indigo-500/10 transition-all">
                    ...
                    <div className="w-full h-1.5 bg-slate-100 dark:bg-slate-700/50 rounded-full mt-5 overflow-hidden">
                        <motion.div initial={{ width: 0 }} animate={{ width: `${details.approvalRating}%` }} transition={{ duration: 1, ease: "easeOut", delay: 0.2 }} className="h-full bg-blue-500" />
                    </div>
                </motion.div>

                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3, duration: 0.5 }} whileHover={{ y: -5 }} className="bg-white dark:bg-slate-800 p-8 rounded-[2rem] border border-slate-200 dark:border-slate-700/60 shadow-sm dark:shadow-none flex flex-col items-center justify-center text-center group hover:shadow-xl dark:hover:shadow-indigo-500/10 transition-all">
                    ...
                    <div className="w-full h-1.5 bg-slate-100 dark:bg-slate-700/50 rounded-full mt-5 overflow-hidden">
                        <motion.div initial={{ width: 0 }} animate={{ width: `${details.completionRate}%` }} transition={{ duration: 1, ease: "easeOut", delay: 0.3 }} className="h-full bg-emerald-500" />
                    </div>
            ...
            <div className="mt-8">
                <h3 className="text-2xl font-black text-slate-900 dark:text-white mb-6 uppercase tracking-tight flex items-center gap-3">
                    ...
                    <span className="px-3 py-1 bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-300 rounded-full text-xs font-bold">{agencyProjects.length}</span>
                </h3>

                {agencyProjects.length === 0 ? (
                    <div className="w-full bg-slate-50 dark:bg-slate-800/40 rounded-3xl p-12 text-center border border-slate-200 dark:border-slate-700/60 border-dashed">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                        {agencyProjects.map((project, index) => (
                            <motion.div key={project.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1, duration: 0.4 }} className="flex">
                                <Link to={`/project/${project.id}`} className="w-full group bg-white dark:bg-slate-800 rounded-[2rem] border border-slate-200/60 dark:border-slate-700/60 overflow-hidden shadow-sm dark:shadow-none hover:shadow-xl dark:hover:shadow-indigo-500/10 transition-all duration-300 hover:-translate-y-2 flex flex-col">
                                    <div className={`h-36 bg-gradient-to-br ${project.ui.bg} relative overflow-hidden flex flex-col items-center justify-center`}>
                                        ...
                                        <div className="absolute top-3 left-3 bg-white/90 dark:bg-black/40 backdrop-blur-sm px-3 py-1 rounded-full text-[9px] font-black text-slate-800 dark:text-white uppercase tracking-wider shadow-sm">{project.category || "General"}</div>
                                    </div>
                                    ...
                                    <div className="p-5 flex flex-col flex-grow">
                                        ...
                                        <div className="mt-auto pt-4 border-t border-slate-100 dark:border-slate-700/50 flex justify-between items-center text-xs font-bold text-slate-500 dark:text-slate-400">
                                            <span>👍 {project.likes || 0}</span>
                                            <span className="text-indigo-500 dark:text-indigo-400 group-hover:translate-x-1 transition-transform">View Details →</span>
                                        </div>
                                    </div>
                                </Link>
                            </motion.div>
