...
export default function Explore() {
  ...
  return (
    <div className="w-full min-h-screen flex flex-col relative overflow-x-hidden font-sans pb-24">
      ...
      <div className="w-full max-w-[90rem] mx-auto px-4 sm:px-6 lg:px-8 py-10 md:py-16 flex flex-col gap-10 md:gap-14 relative z-10">
        <section className="flex flex-col gap-6">
          ...
          <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1, duration: 0.5, type: "spring", bounce: 0.4 }} className="text-4xl md:text-6xl lg:text-7xl font-black tracking-tighter leading-[1.1] drop-shadow-sm">
                ...
                {projects.map((project, idx) => {
                  ...
                  return (
                    <motion.div key={project.id} layout initial={{ opacity: 0, y: 40, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, scale: 0.9 }} transition={{ delay: idx * 0.05, duration: 0.4, type: "spring", bounce: 0.3 }} className="group relative bg-white/80 dark:bg-slate-900/60 backdrop-blur-xl rounded-[2rem] border border-slate-200/60 dark:border-slate-800/60 shadow-lg shadow-slate-200/40 dark:shadow-[0_8px_30px_rgb(0,0,0,0.15)] hover:shadow-2xl dark:hover:shadow-[0_20px_40px_rgba(99,102,241,0.1)] transition-all duration-500 flex flex-col transform-gpu hover:-translate-y-2">
                      ...
                      <Link to={`/project/${project.id}`} className={`w-full aspect-[4/3] rounded-t-[2rem] rounded-b-[1rem] bg-gradient-to-br ${project.imageGradient} relative overflow-hidden flex flex-col items-center justify-center shadow-inner block group/image`}>
                        <div className="absolute inset-0 opacity-30 group-hover/image:scale-125 group-hover/image:rotate-2 transition-transform duration-1000 ease-out mix-blend-overlay">
                        ...
                        <div className="absolute top-4 left-4 bg-white/20 backdrop-blur-md px-3.5 py-1.5 rounded-full text-white font-black tracking-widest uppercase text-[9px] md:text-[10px] shadow-sm border border-white/20 group-hover/image:scale-105 transition-transform duration-300">{project.category || "General"}</div>
                      ...
                      <div className="p-4 md:p-6 flex flex-col flex-grow gap-3 md:gap-4 relative z-10 bg-transparent">
                        <div>
                        ...
                        <div className="flex flex-col gap-4 mt-2 pt-4 border-t border-slate-100 dark:border-slate-800/60">
                          <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-1 bg-slate-50 dark:bg-slate-800/50 p-1.5 rounded-[1rem] border border-slate-100 dark:border-slate-700/50">
                              <motion.button whileHover={{ scale: 1.15, rotate: -10 }} whileTap={{ scale: 0.9 }} onClick={() => handleReaction(project.id, 'likes')} className="flex items-center gap-1.5 px-2 md:px-2.5 py-1.5 rounded-lg hover:bg-white dark:hover:bg-slate-700 text-[10px] md:text-[11px] font-black text-slate-500 dark:text-slate-400 hover:text-emerald-600 dark:hover:text-emerald-400 transition-all shadow-sm">👍 <span>{project.likes || 0}</span></motion.button>
                              <div className="w-px h-3 bg-slate-200 dark:bg-slate-700"></div>
                              <motion.button whileHover={{ scale: 1.15 }} whileTap={{ scale: 0.9 }} onClick={() => handleReaction(project.id, 'neutrals')} className="flex items-center gap-1.5 px-2 md:px-2.5 py-1.5 rounded-lg hover:bg-white dark:hover:bg-slate-700 text-[10px] md:text-[11px] font-black text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition-all shadow-sm">😐 <span>{project.neutrals || 0}</span></motion.button>
                              <div className="w-px h-3 bg-slate-200 dark:bg-slate-700"></div>
                              <motion.button whileHover={{ scale: 1.15, rotate: 10 }} whileTap={{ scale: 0.9 }} onClick={() => handleReaction(project.id, 'unlikes')} className="flex items-center gap-1.5 px-2 md:px-2.5 py-1.5 rounded-lg hover:bg-white dark:hover:bg-slate-700 text-[10px] md:text-[11px] font-black text-slate-500 dark:text-slate-400 hover:text-rose-600 dark:hover:text-rose-400 transition-all shadow-sm">👎 <span>{project.unlikes || 0}</span></motion.button>
                            </div>
                          </div>

                          <Link to={`/project/${project.id}`} className="w-full bg-indigo-50 dark:bg-indigo-500/10 hover:bg-indigo-100 dark:hover:bg-indigo-500/20 text-indigo-700 dark:text-indigo-400 text-[10px] md:text-xs font-black uppercase tracking-widest py-3 md:py-3.5 rounded-xl transition-all duration-300 text-center flex justify-center items-center gap-2 group/btn">
                            View Project
                            <svg className="w-3.5 h-3.5 md:w-4 md:h-4 transform group-hover/btn:translate-x-1 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M17 8l4 4m0 0l-4 4m4-4H3" /></svg>
                          </Link>
                        </div>
                      </div>
                    </motion.div>
