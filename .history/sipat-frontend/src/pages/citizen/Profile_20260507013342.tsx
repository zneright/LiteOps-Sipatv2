...
export default function ProjectView() {
  ...
  const [isAgencyFollowed, setIsAgencyFollowed] = useState(false);
    ...
    if (user?.email) {
      ...
      if (orgName) {
        fetch(`http://localhost:8080/api/users/followed-agencies?email=${user.email}`)
          .then(res => res.json())
          .then(data => { if (data.followed_agencies) setIsAgencyFollowed(data.followed_agencies.includes(orgName)); })
          .catch(console.error);
      }
    }
  }, [user, id, orgName]);
  ...
  const toggleFollowAgency = async (e: React.MouseEvent) => {
    e.preventDefault(); e.stopPropagation();
    if (!user?.email) return alert("Please log in to follow agencies.");
    setIsAgencyFollowed(!isAgencyFollowed);
    try {
      await fetch("http://localhost:8080/api/users/toggle-follow-agency", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email: user.email, agency_name: orgName })
      });
        ...
        <div className={` ${isReply ? 'ml-4 sm:ml-8 mt-3' : 'mt-6'} p-4 sm:p-5 rounded-2xl sm:rounded-3xl border shadow-sm transition-all ${isGhost ? 'bg-red-50 border-red-200' : comment.ai_match_score >= 40 ? 'bg-emerald-50 border-emerald-200' : 'bg-white border-slate-200'} `}>
          <div className="flex justify-between items-start mb-3">
            <div className="flex flex-col gap-0.5">
              <Link to={`/profile/${encodeURIComponent(comment.author_name)}`} className="font-bold text-xs sm:text-sm text-slate-900 hover:text-indigo-600 transition-colors">{comment.author_name}</Link>
      ...
      <div className="w-full md:w-1/3 flex flex-col gap-6">
        <div>
          ...
          <div className="flex justify-between items-start gap-4 mb-2">
            ...
            <button onClick={toggleFollowProject} className={`shrink-0 w-10 h-10 rounded-full flex items-center justify-center transition-all ${isFollowed ? 'bg-rose-100 text-rose-500 shadow-sm border border-rose-200' : 'bg-slate-100 text-slate-400 hover:bg-slate-200 hover:text-rose-400'}`}>
          ...
          {/* 🚀 FOLLOW AGENCY CARD */}
          <div className="group flex items-center gap-4 bg-indigo-50 p-3 rounded-2xl border border-indigo-100 mb-6 transition-all duration-300 relative overflow-hidden">
            <Link to={`/agency-profile/${encodeURIComponent(orgName)}`} className="flex items-center gap-4 flex-1">
              <div className="w-12 h-12 rounded-xl bg-white flex items-center justify-center text-xl font-black text-indigo-600 shadow-sm border border-slate-100 group-hover:scale-110 transition-transform">
                {orgName.charAt(0).toUpperCase()}
              </div>
              <div className="flex flex-col">
                <span className="text-[9px] font-black text-indigo-400 uppercase tracking-widest flex items-center gap-1">
                  <svg className="w-3 h-3 text-emerald-500" fill="currentColor" viewBox="0 0 24 24"><path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41L9 16.17z" /></svg> Managed By Official Agency
                </span>
                <span className="text-sm font-bold text-slate-900 group-hover:text-indigo-600 transition-colors">{orgName}</span>
              </div>
            </Link>
            <button onClick={toggleFollowAgency} className={`px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-colors shadow-sm border z-10 ${isAgencyFollowed ? 'bg-indigo-600 text-white border-indigo-700' : 'bg-white text-indigo-600 border-indigo-200 hover:bg-indigo-100'}`}>
              {isAgencyFollowed ? "Following" : "Follow"}
            </button>
          </div>
          ...
          <div className="flex flex-col gap-1.5 mb-2 bg-white p-4 rounded-2xl border border-slate-200/60 shadow-sm">
            ...
            <div className="w-full h-2 bg-slate-100 rounded-full overflow-hidden mb-3"><motion.div initial={{ width: 0 }} animate={{ width: `${percentage}%` }} className={`h-full rounded-full transition-all duration-1000 ease-out ${percentage === 100 ? 'bg-emerald-500' : 'bg-indigo-600'}`} /></div>
      ...
      <div className="w-full md:w-2/3 flex flex-col gap-6">
        <div className="bg-white rounded-3xl p-4 sm:p-6 border border-slate-200 shadow-sm relative overflow-hidden">
          ...
          <form onSubmit={submitComment} className="flex flex-col gap-4 relative z-10">
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-4">
              <label className="flex items-center gap-2 cursor-pointer text-sm font-bold text-slate-600"><input type="checkbox" checked={isAnonymous} onChange={(e) => setIsAnonymous(e.target.checked)} className="w-4 h-4 text-indigo-600 rounded border-slate-300 focus:ring-indigo-500" /> Post Anonymously</label>
            ...
            <textarea ref={commentInputRef} required placeholder="Explain what the photo shows..." rows={3} value={textContent} onChange={(e) => setTextContent(e.target.value)} className="w-full p-4 rounded-xl bg-slate-50 border border-slate-200 text-sm outline-none focus:ring-2 focus:ring-indigo-500/50 transition-shadow" />
        ...
        {topLevelComments.length === 0 ? (
          <div className="p-6 sm:p-10 text-center text-slate-400 bg-slate-50 rounded-3xl border border-slate-200 border-dashed">
            <div className="text-4xl mb-3 opacity-50">📸</div><p className="text-sm font-bold text-slate-500">No proof submitted yet.</p><p className="text-xs mt-1">Be the first to upload evidence for this phase.</p>
          </div>
