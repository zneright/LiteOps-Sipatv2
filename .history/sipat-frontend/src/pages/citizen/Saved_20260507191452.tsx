...
export default function Saved() {
  ...
  useEffect(() => {
    const fetchSavedProjects = async () => {
      try {
        setIsLoading(true); // Ensure loading is true when key changes
        ...
        // 3. Filter ONLY saved ones (Bulletproof type checking)
        const filteredAndProcessed = allProjects
          .filter((p) => ids.some(savedId => String(savedId) === String(p.id)))
          .map((p) => {
    ...
    // Only run this if we are confident the user object has initialized.
    // If useAuth returns undefined while loading, we wait.
    if (user !== undefined) {
      fetchSavedProjects();
    }
  }, [storageKey, user]);
