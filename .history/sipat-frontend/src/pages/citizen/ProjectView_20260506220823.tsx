...
export default function ProjectView() {
    ...
    const verifyPhotoWithAI = async (photoFile: File, userComment: string, projectContext: string) => {
        try {
            ...
            // 🚀 FIX 1: Using the exact, correct model name
            ...
            Task: Analyze both the attached photo AND the citizen's comment to verify the project's status.

            1. match_score (0-100): How well does the visual evidence in the photo support the Official Project Details? Use the Citizen's Comment to understand what the photo is showing, but always prioritize visual proof over claims.
            2. is_ghost_project (boolean): Does the photo AND comment expose this as a fake, abandoned, or "ghost" project? Set to true ONLY if the citizen's comment suggests abandonment AND the photo visually proves it (e.g., empty dirt lots, severe decay, zero construction). If the photo shows a completed or active project, this must be false regardless of what the text says.
            3. reason (string): Provide a short, 1-sentence explanation of why you gave this score.
            `;
            const result = await model.generateContent([prompt, { inlineData } as any]);
            return JSON.parse(result.response.text());
        } catch (e: any) {
            // 🚀 FIX 2: Actually alert the user so you know it failed!
    ...
    const submitComment = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const dbRes = await fetch("http://localhost:8080/api/comments", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                    project_id: id,
            ...
            // 🚀 FIX 3: Catch block for the entire submission process
    ...
    const renderComment = (comment: any, isReply = false) => {
        ...
        // 🚀 FIX 1: Safely convert the MySQL string "0" or "1" into a true Javascript boolean
        const isGhost = Number(comment.is_ghost_alert) === 1;

        return (
            // 🚀 FIX 2: Use the new `isGhost` variable for the background color
            <div key={comment.id} className={`${isReply ? 'ml-8 md:ml-12 mt-4 pl-4 border-l-2 border-slate-200 dark:border-slate-800' : 'p-6 rounded-3xl border shadow-sm mt-6'} ${isGhost ? 'bg-red-50 dark:bg-red-500/10 border-red-200' : comment.ai_match_score >= 40 ? 'bg-emerald-50 dark:bg-emerald-500/10 border-emerald-200' : 'bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800'}`}>
                ...
                <div className="flex justify-between items-start mb-4">
                    ...
                    <div className="flex flex-col items-end">
                        {/* 🚀 FIX 3: Use the new `isGhost` variable for the red badge */}
                        {isGhost ? (
