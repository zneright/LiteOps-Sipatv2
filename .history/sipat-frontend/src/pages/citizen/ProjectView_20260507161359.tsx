            ...
            <div className="relative max-w-[90rem] mx-auto py-8 sm:py-12 px-4 sm:px-6 lg:px-8 flex flex-col xl:flex-row gap-8 xl:gap-12">
                ...
                <div className="w-full xl:w-[55%] flex flex-col gap-8">
                    <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ type: "spring", bounce: 0.4 }} className="flex flex-col gap-8">
                        ...
                        {/* Completely Grid-based Phases Selection - No Scrolling */}
                            ...
                            <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 w-full">
                                <button onClick={() => setActiveTab("Overall Project")} className={`relative flex flex-col justify-between p-4 min-h-[110px] w-full rounded-[1.25rem] sm:rounded-[1.5rem] transition-all duration-300 transform-gpu overflow-hidden border ${activeTab === "Overall Project" ? "text-white dark:text-slate-900 border-transparent shadow-[0_8px_20px_rgba(99,102,241,0.3)] scale-[1.02] z-10" : "bg-white/70 dark:bg-slate-900/60 backdrop-blur-xl border-white/80 dark:border-slate-700/50 text-slate-500 dark:text-slate-400 hover:bg-white dark:hover:bg-slate-800 shadow-sm hover:shadow-md hover:-translate-y-1"}`}>
                                    ...
                                    <div className="flex justify-between items-start w-full relative z-10">
                                        <div className={`w-8 h-8 rounded-full flex items-center justify-center shadow-inner ${activeTab === "Overall Project" ? "bg-white/20 dark:bg-black/10 text-white dark:text-slate-900" : "bg-slate-100 dark:bg-slate-800 text-indigo-500"}`}>
                                    ...
                                    <div className="text-left relative z-10 mt-auto">
                                        ...
                                        <span className="block text-xs sm:text-sm font-black leading-tight truncate">Overall Project</span>
                                    </div>
                                </button>
                                ...
                                {project.phases?.map((p: any, index: number) => {
                                    ...
                                    return (
                                        <button key={p.title} onClick={() => setActiveTab(p.title)} className={`relative flex flex-col justify-between p-4 min-h-[110px] w-full rounded-[1.25rem] sm:rounded-[1.5rem] transition-all duration-300 transform-gpu overflow-hidden border ${isActive ? "text-white dark:text-slate-900 border-transparent shadow-[0_8px_20px_rgba(99,102,241,0.3)] scale-[1.02] z-10" : "bg-white/70 dark:bg-slate-900/60 backdrop-blur-xl border-white/80 dark:border-slate-700/50 text-slate-500 dark:text-slate-400 hover:bg-white dark:hover:bg-slate-800 shadow-sm hover:shadow-md hover:-translate-y-1"}`}>
                                            {isActive && (
                                                <motion.div layoutId="activePhaseGrid" className="absolute inset-0 bg-gradient-to-br from-indigo-500 to-purple-600 rounded-[1.25rem] sm:rounded-[1.5rem] -z-10" transition={{ type: "spring", bounce: 0.2, duration: 0.6 }} />
                                            )}
                                            <div className="flex justify-between items-start w-full relative z-10">
                                                <div className={`text-xl sm:text-2xl font-black italic opacity-30 ${isActive ? "text-white dark:text-slate-900" : "text-slate-400"}`}>
                                                    #{index + 1}
                                                </div>
                                                {isCompleted && (
                                                    <div className={`w-6 h-6 rounded-full flex items-center justify-center shadow-sm ${isActive ? "bg-white text-emerald-500 dark:bg-slate-900 dark:text-emerald-400" : "bg-emerald-100 text-emerald-600 dark:bg-emerald-500/20 dark:text-emerald-400"}`}>
                                                        <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" /></svg>
                                                    </div>
                                                )}
                                            </div>
                                            <div className="text-left relative z-10 mt-auto">
                                                <span className={`block text-[9px] uppercase font-black tracking-widest mb-0.5 ${isActive ? "opacity-80 text-indigo-100 dark:text-indigo-900" : "text-slate-400"}`}>{isCompleted ? "Completed" : "In Progress"}</span>
                                                <span className="block text-xs sm:text-sm font-black leading-tight line-clamp-2">{p.title}</span>
                                            </div>
                                        </button>
                                    );
                                })}
