                  ...
                  return (
                    <motion.div key={project.id} layout initial={{ opacity: 0, y: 40, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, scale: 0.9 }} transition={{ delay: idx * 0.05, duration: 0.4, type: "spring", bounce: 0.3 }} className="group relative bg-white/80 dark:bg-slate-900/60 backdrop-blur-xl rounded-[2rem] border border-slate-200/60 dark:border-slate-800/60 shadow-lg shadow-slate-200/40 dark:shadow-[0_8px_30px_rgb(0,0,0,0.15)] hover:shadow-2xl dark:hover:shadow-[0_20px_40px_rgba(99,102,241,0.1)] transition-all duration-500 flex flex-col transform-gpu hover:-translate-y-2">
                      ...
                      <Link to={`/project/${project.id}`} className={`w-full aspect-[4/3] rounded-t-[2rem] rounded-b-[1rem] bg-gradient-to-br ${project.imageGradient} relative overflow-hidden flex flex-col items-center justify-center shadow-inner block group/image`}>
                        ...
                        <div className="absolute top-4 left-4 bg-white/20 backdrop-blur-md px-3.5 py-1.5 rounded-full text-white font-black tracking-widest uppercase text-[9px] md:text-[10px] shadow-sm border border-white/20 group-hover/image:scale-105 transition-transform duration-300 z-20">{project.category || "General"}</div>
                        ...
                        <div className="flex flex-col gap-4 mt-2 pt-4 border-t border-slate-100 dark:border-slate-800/60">
                          <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-1 bg-slate-50 dark:bg-slate-800/50 p-1.5 rounded-[1rem] border border-slate-100 dark:border-slate-700/50 shadow-sm relative z-30">
                              <motion.button whileHover={{ scale: 1.15, rotate: -10 }} whileTap={{ scale: 0.9 }} onClick={(e) => handleReaction(e, project.id, 'likes')} className="flex items-center gap-1.5 px-2 md:px-2.5 py-1.5 rounded-lg hover:bg-white dark:hover:bg-slate-700 text-[10px] md:text-[11px] font-black text-slate-500 dark:text-slate-400 hover:text-emerald-600 dark:hover:text-emerald-400 transition-all shadow-sm relative z-30">👍 <span>{project.likes || 0}</span></motion.button>
                              ...
                              <div className="w-px h-3 bg-slate-200 dark:bg-slate-700"></div>
                              <motion.button whileHover={{ scale: 1.15, rotate: 10 }} whileTap={{ scale: 0.9 }} onClick={(e) => handleReaction(e, project.id, 'unlikes')} className="flex items-center gap-1.5 px-2 md:px-2.5 py-1.5 rounded-lg hover:bg-white dark:hover:bg-slate-700 text-[10px] md:text-[11px] font-black text-slate-500 dark:text-slate-400 hover:text-rose-600 dark:hover:text-rose-400 transition-all shadow-sm relative z-30">👎 <span>{project.unlikes || 0}</span></motion.button>
                            </div>
                          </div>
                        </div>
                      </div>
