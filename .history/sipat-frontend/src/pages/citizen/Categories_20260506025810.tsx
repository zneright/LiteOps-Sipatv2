...
// 1. Keep the visual data static (Icons, Gradients, Descriptions)
// We remove the hardcoded 'count' from here because it will come from the DB.
...
export default function Categories() {
  // 2. Setup state to hold the merged data
  const [categories, setCategories] = useState<Category[]>([]);
  const [isLoading, setIsLoading] = useState(true);

  // 3. Fetch from the DB and tally the counts
  useEffect(() => {
    const fetchAndCountCategories = async () => {
      try {
        // Fetch all projects using your existing CodeIgniter GET endpoint
        const response = await fetch("http://localhost:8080/api/projects");
        if (!response.ok) throw new Error("Failed to fetch projects");

        const projects = await response.json();

        // Create a tally dictionary (e.g., { "Infrastructure": 12, "Technology": 5 })
        ...
        // Merge the tallies with our visual metadata
        const mergedCategories = categoryMetadata.map((meta) => ({
          ...meta,
          count: categoryCounts[meta.name] || 0, // Default to 0 if no projects exist for it
        }));
        ...
        // Fallback: If DB fails, show 0 for everything so the UI doesn't break
