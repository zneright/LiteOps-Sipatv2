...
export default function Profile() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        if (userId) {
          const userRes = await fetch(`${API_URL}/api/users?firebase_uid=${userId}`);
        ...
        const commentsPromises = projects.map((p: any) => fetch(`${API_URL}/api/comments/${p.id}`).then(res => res.json()));
    ...
    <div className="w-full min-h-screen flex flex-col relative overflow-x-hidden font-sans pb-24 bg-slate-50 dark:bg-slate-950">
      ...
      <div className="max-w-[85rem] mx-auto w-full px-4 sm:px-6 lg:px-8 py-8 md:py-12 flex flex-col gap-8 md:gap-12 relative z-10">
        ...
        <motion.div variants={staggerContainer} initial="hidden" animate="show" className="w-full flex flex-col gap-3">
          ...
          <div className="relative">
            <div className="absolute left-0 top-0 bottom-0 w-12 bg-gradient-to-r from-slate-50 dark:from-slate-950 to-transparent z-10 pointer-events-none sm:hidden"></div>
            <div className="absolute right-0 top-0 bottom-0 w-12 bg-gradient-to-l from-slate-50 dark:from-slate-950 to-transparent z-10 pointer-events-none sm:hidden"></div>
