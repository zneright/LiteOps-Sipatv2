...
export default function ProjectView() {
    ...
    const GEMINI_API_KEY = "AIzaSyDiX2AbY2y0CvDUTh3u6Bgg9E6aWcztos4"; // Remember to secure this!
    ...
    // 🚀 UPGRADED: Now handles BOTH Text-Only and Image+Text comments!
    const verifyContentWithAI = async (userComment: string, projectContext: string, photoFile: File | null) => {
        try {
            ...
            if (photoFile) {
                // 📸 SCENARIO A: They uploaded a photo
                ...
                // ✍️ SCENARIO B: Text-Only Comment (No Photo)
                prompt = `
                You are an AI auditing public infrastructure projects. 
                Official Project Details: "${projectContext}"
                Citizen's Comment: "${userComment}"
            ...
            // 🚨 NEW: Explicit warning if the AI fails!
    ...
    const submitComment = async (e: React.FormEvent) => {
        ...
        try {
            // 1. Only upload to Cloudinary if there is a file
            if (file) {
            ...
            // 2. Get the correct context (Overall vs Specific Phase)
            ...
            // 3. 🚀 FIX: Run the AI NO MATTER WHAT! (Passing the file or null)
    ...
    return (
        <div className="relative max-w-5xl mx-auto py-10 px-4 flex flex-col md:flex-row gap-10">
            ...
            <div className="w-full md:w-1/3 flex flex-col gap-6">
                <div>
                    {/* 🚀 NEW: Smart Back Button! Checks if role is agency or citizen */}
                    <Link
                    ...
                    >
                        ...
                        {role === 'agency' ? "Back to Feedback Dashboard" : "Back to Feed"}
                    </Link>
