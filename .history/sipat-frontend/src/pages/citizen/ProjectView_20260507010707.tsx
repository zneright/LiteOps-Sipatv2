        ...
        <div className="relative max-w-5xl mx-auto py-6 sm:py-10 px-4 flex flex-col md:flex-row gap-6 md:gap-10">
            <AnimatePresence>
                {role === 'agency' && (
                    ...
                    >
                        <button onClick={() => navigate('/agency/feedback')} className="w-full justify-center bg-slate-900 text-white dark:bg-white dark:text-slate-900 px-4 sm:px-6 py-2.5 sm:py-3 rounded-full shadow-2xl flex items-center gap-2 text-[10px] sm:text-sm font-black tracking-wider border border-slate-700 transition-transform hover:scale-105">
            ...
            <div className="w-full md:w-1/3 flex flex-col gap-6">
                <div>
                    <Link to={role === 'agency' ? "/agency/feedback" : "/explore"} className="inline-flex items-center gap-2 text-sm font-bold text-slate-500 hover:text-indigo-600 transition-colors mb-4">
                        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M10 19l-7-7m0 0l7-7m-7 7h18" /></svg>
                        {role === 'agency' ? "Back to Dashboard" : "Back to Feed"}
                    </Link>

                    <h1 className="text-2xl sm:text-3xl font-black mb-4 break-words leading-tight">{project.title}</h1>

                    {/* 🚀 PREMIUM OFFICIAL AGENCY CARD */}
                    <Link to={`/agency-profile/${encodeURIComponent(orgName)}`} className="group flex items-center gap-4 bg-indigo-50 dark:bg-indigo-500/10 p-3 rounded-2xl border border-indigo-100 dark:border-indigo-500/20 mb-6 hover:shadow-md hover:bg-white dark:hover:bg-slate-800 transition-all duration-300">
                        <div className="w-12 h-12 rounded-xl bg-white dark:bg-slate-950 flex items-center justify-center text-xl font-black text-indigo-600 shadow-sm border border-slate-100 dark:border-slate-800 group-hover:scale-110 transition-transform">
                        ...
                        <div className="flex flex-col">
                            <span className="text-[9px] font-black text-indigo-400 uppercase tracking-widest flex items-center gap-1">
                                ...
                                Managed By Official Agency
                            </span>
                            <span className="text-sm font-bold text-slate-900 dark:text-white group-hover:text-indigo-600 transition-colors">{orgName}</span>
                        </div>
                    </Link>

                    <div className="flex flex-wrap gap-3 sm:gap-4 mb-5">
                        <div className="bg-emerald-50 dark:bg-emerald-500/10 px-3 py-1.5 rounded-lg border border-emerald-200 flex-1">
                        ...
                        <div className="bg-slate-50 dark:bg-slate-800/50 px-3 py-1.5 rounded-lg border border-slate-200 flex-1">
                    ...
                    <p className="text-slate-500 dark:text-slate-400 text-sm leading-relaxed mt-4 bg-slate-50 dark:bg-slate-900/50 p-4 rounded-2xl">
                        {isOverall ? project.description : currentContextData?.description}
                    </p>
                </div>

                {/* Horizontal Phase Selector */}
                <div className="bg-slate-50 dark:bg-slate-900/50 rounded-2xl p-2 flex flex-row md:flex-col overflow-x-auto snap-x scrollbar-hide gap-2 border border-slate-200 mt-2 shadow-inner">
                    <h3 className="hidden md:block text-xs font-black text-slate-400 uppercase tracking-wider p-2">Filter by Phase</h3>

                    <button onClick={() => setActiveTab("Overall Project")} className={`shrink-0 md:w-full text-left px-4 py-3 rounded-xl text-sm font-bold transition-all snap-center ${activeTab === "Overall Project" ? "bg-white shadow-sm border-slate-200 text-indigo-600 scale-[1.02]" : "text-slate-500 hover:bg-slate-100 border-transparent border"}`}>
                        Overall Project
                    </button>
            ...
            <div className="w-full md:w-2/3 flex flex-col gap-6">
                <div className="bg-white dark:bg-slate-900 rounded-3xl p-4 sm:p-6 border border-slate-200 shadow-sm relative overflow-hidden">
                    <div className="absolute top-0 right-0 w-32 h-32 bg-indigo-500/5 rounded-full blur-3xl pointer-events-none" />
                    <h3 className="text-base sm:text-lg font-black mb-1 text-slate-900 dark:text-white relative z-10">Submit Proof for: <span className="text-indigo-600">{activeTab}</span></h3>
                    ...
                    <form onSubmit={submitComment} className="flex flex-col gap-4 relative z-10">
                        ...
                        <textarea
                            ...
                            className="w-full p-4 rounded-xl bg-slate-50 dark:bg-slate-950/50 border border-slate-200 text-sm outline-none focus:ring-2 focus:ring-indigo-500/50 transition-shadow"
                        />
                ...
                <div className="flex items-center justify-between mt-2">
                    <h3 className="text-lg sm:text-xl font-black text-slate-900 dark:text-white">Community Evidence</h3>
                    <span className="bg-slate-100 dark:bg-slate-800 text-slate-500 font-bold px-3 py-1 rounded-full text-xs">{phaseComments.length} entries</span>
                </div>
                ...
                {topLevelComments.length === 0 ? (
                    <div className="p-6 sm:p-10 text-center text-slate-400 bg-slate-50 dark:bg-slate-900/50 rounded-3xl border border-slate-200 border-dashed">
                        ...
                        <p className="text-sm font-bold text-slate-500">No proof submitted yet.</p>
                        <p className="text-xs mt-1">Be the first to upload evidence for this phase.</p>
                    </div>
                ) : (
