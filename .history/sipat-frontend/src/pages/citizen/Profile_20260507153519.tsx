...
export default function Profile() {
  ...
  return (
    <div className="w-full min-h-screen flex flex-col relative overflow-x-hidden font-sans pb-24 bg-slate-50 dark:bg-slate-950">
      ...
      <div className="max-w-6xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-8 md:py-12 flex flex-col gap-8 md:gap-10 relative z-10">
        ...
        <motion.div initial={{ opacity: 0, y: 30, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ duration: 0.6, type: "spring", bounce: 0.4 }} className="relative bg-white/70 dark:bg-slate-900/60 backdrop-blur-2xl rounded-[2.5rem] md:rounded-[3rem] border border-white/80 dark:border-slate-700/50 shadow-xl dark:shadow-[0_8px_30px_rgb(0,0,0,0.3)] overflow-hidden">
          ...
          <div className="absolute top-0 left-0 w-full h-40 sm:h-56 bg-slate-100 dark:bg-slate-950 overflow-hidden">
            ...
            <div className="absolute inset-0 bg-[url('data:image/svg+xml;base64,PHN2ZyB3aWR0aD0iMjAiIGhlaWdodD0iMjAiIHhtbG5zPSJodHRwOi8vd3d3LnczLm9yZy8yMDAwL3N2ZyI+PGNpcmNsZSBjeD0iMTAiIGN5PSIxMCIgcj0iMSIgZmlsbD0icmdiYSgxMDAsMTAwLDEwMCwwLjEpIi8+PC9zdmc+')] [mask-image:linear-gradient(to_bottom,white,transparent)]" />
          </div>
          ...
          >
            <div className={`w-16 h-16 sm:w-20 sm:h-20 bg-gradient-to-br ${activeRank.grad} rounded-[1.25rem] sm:rounded-[1.5rem] p-1 shadow-2xl shadow-${activeRank.color.split('-')[1]}-500/50 cursor-help transform transition-transform group-hover:scale-110 group-hover:rotate-3`}>
              <div className="w-full h-full bg-white dark:bg-slate-900 rounded-xl sm:rounded-[1.25rem] flex items-center justify-center text-3xl sm:text-4xl">
                {activeRank.icon}
              </div>
            </div>
            <div className="absolute -bottom-8 opacity-0 group-hover:opacity-100 transition-opacity bg-slate-900 dark:bg-white text-white dark:text-slate-900 text-[10px] sm:text-xs font-black px-3 sm:px-4 py-1.5 rounded-full whitespace-nowrap shadow-xl pointer-events-none transform translate-y-2 group-hover:translate-y-0 duration-300 uppercase tracking-widest">
              {activeRank.title}
            </div>
          </motion.div>
          ...
          <div className="px-6 sm:px-10 pb-8 sm:pb-10 pt-24 sm:pt-36 relative z-10 flex flex-col sm:flex-row items-center sm:items-end gap-6 md:gap-8 text-center sm:text-left">
            <div className="relative group">
              <div className="w-32 h-32 sm:w-40 sm:h-40 rounded-[2rem] sm:rounded-[2.5rem] bg-white/50 dark:bg-slate-800/50 p-2 backdrop-blur-xl shadow-2xl shrink-0 -mt-24 sm:-mt-16 relative z-10 transition-transform duration-500 group-hover:scale-105">
                <div className={`w-full h-full rounded-[1.75rem] sm:rounded-[2.25rem] bg-gradient-to-br ${activeRank.grad} p-[3px]`}>
                  <div className="w-full h-full rounded-[1.5rem] sm:rounded-[2rem] bg-white dark:bg-slate-900 flex items-center justify-center text-5xl sm:text-6xl font-black text-slate-800 dark:text-white shadow-inner">
            ...
            <div className="flex-1 w-full flex flex-col items-center sm:items-start gap-2">
              <h1 className="text-4xl sm:text-5xl font-black text-slate-900 dark:text-white tracking-tighter leading-none">{user?.name || "Citizen"}</h1>
            ...
            <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => setIsEditing(true)} className="px-6 md:px-8 py-3 md:py-3.5 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-2xl font-black text-xs md:text-sm uppercase tracking-widest shadow-xl hover:shadow-2xl transition-all w-full sm:w-auto mt-4 sm:mt-0 flex items-center justify-center gap-2">
        ...
        <motion.div variants={staggerContainer} initial="hidden" animate="show" className="w-full flex flex-col gap-3">
          <div className="flex items-center justify-between px-2">
            ...
            <span className="text-[10px] md:text-xs font-black bg-slate-200/50 dark:bg-slate-800 px-3 py-1 rounded-full text-slate-600 dark:text-slate-300">{ALL_RANKS.filter(r => currentCount >= r.req).length} / {ALL_RANKS.length} Unlocked</span>
          </div>

          <div className="relative">
            <div className="absolute left-0 top-0 bottom-0 w-8 bg-gradient-to-r from-slate-50 dark:from-slate-950 to-transparent z-10 pointer-events-none sm:hidden"></div>
            <div className="absolute right-0 top-0 bottom-0 w-8 bg-gradient-to-l from-slate-50 dark:from-slate-950 to-transparent z-10 pointer-events-none sm:hidden"></div>

            <div className="flex gap-3 sm:gap-4 overflow-x-auto pb-6 pt-2 scrollbar-hide snap-x px-2">
              {ALL_RANKS.map((rank) => {
                const isUnlocked = currentCount >= rank.req;
                return (
                  <motion.div key={rank.id} variants={fadeUp} whileHover={isUnlocked ? { y: -8, scale: 1.05 } : {}} className={`shrink-0 snap-center relative flex flex-col items-center justify-center p-4 rounded-[1.5rem] md:rounded-[2rem] w-24 h-24 sm:w-28 sm:h-28 transition-all duration-300 ${isUnlocked ? `bg-white/60 dark:bg-slate-900/60 backdrop-blur-md border-2 border-white/80 dark:border-slate-700/50 shadow-lg hover:shadow-xl cursor-help` : 'bg-slate-100/50 dark:bg-slate-800/30 border border-slate-200/50 dark:border-slate-800/50 opacity-60 grayscale'}`}>
                    {isUnlocked && (
                      <div className={`absolute inset-0 bg-gradient-to-br ${rank.grad} opacity-10 rounded-[1.5rem] md:rounded-[2rem] pointer-events-none`} />
                    )}
                    <div className="text-3xl sm:text-4xl mb-1.5 relative z-10 drop-shadow-md">{rank.icon}</div>
                    <div className={`text-[9px] sm:text-[10px] font-black uppercase tracking-widest text-center leading-tight relative z-10 ${isUnlocked ? rank.color : 'text-slate-400'}`}>
                      {rank.title}
                    </div>
                    {!isUnlocked && (
                      <div className="absolute inset-0 bg-slate-900/10 dark:bg-slate-900/40 rounded-[1.5rem] md:rounded-[2rem] flex items-center justify-center backdrop-blur-[1px]">
                        <div className="bg-slate-900/80 dark:bg-slate-800/90 backdrop-blur-md text-white dark:text-slate-300 text-[9px] font-black uppercase tracking-widest px-2.5 py-1 rounded-lg border border-white/10 shadow-lg">Need {rank.req}</div>
                      </div>
                    )}
                  </motion.div>
                );
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-8">
          ...
          <motion.div variants={fadeUp} initial="hidden" animate="show" className="md:col-span-2 bg-white/70 dark:bg-slate-900/60 backdrop-blur-xl p-6 sm:p-10 rounded-[2rem] md:rounded-[2.5rem] border border-white/80 dark:border-slate-700/50 shadow-xl dark:shadow-[0_8px_30px_rgb(0,0,0,0.2)] flex flex-col justify-center relative overflow-hidden group">
            <div className="absolute -right-10 -top-10 w-40 h-40 bg-indigo-500/10 blur-[40px] rounded-full group-hover:bg-indigo-500/20 transition-colors duration-500"></div>

            <div className="flex justify-between items-start mb-8 relative z-10">
              <div>
                <p className="text-[10px] md:text-xs font-black text-slate-400 uppercase tracking-widest mb-1.5 flex items-center gap-2">
            ...
            <div className="w-full relative z-10 bg-slate-50/50 dark:bg-slate-800/30 p-5 md:p-6 rounded-[1.5rem] border border-white/50 dark:border-slate-700/50">
              <div className="flex justify-between items-end mb-3">
                ...
                <span className="text-[10px] sm:text-xs font-black text-slate-400 uppercase tracking-widest bg-white dark:bg-slate-800 px-3 py-1.5 rounded-lg shadow-sm">
          ...
          <motion.div variants={fadeUp} initial="hidden" animate="show" className="md:col-span-1 bg-white/70 dark:bg-slate-900/60 backdrop-blur-xl p-6 sm:p-8 rounded-[2rem] md:rounded-[2.5rem] border border-white/80 dark:border-slate-700/50 shadow-xl dark:shadow-[0_8px_30px_rgb(0,0,0,0.2)] flex flex-col">
            <h2 className="text-[11px] md:text-xs font-black text-slate-400 uppercase tracking-widest mb-5 flex items-center gap-2">
            ...
            <div className="bg-slate-50/50 dark:bg-slate-800/30 p-4 rounded-2xl border border-white/40 dark:border-slate-700/50 flex-1 relative">
              <span className="absolute top-2 left-2 text-4xl text-slate-200 dark:text-slate-700 opacity-50 font-serif">"</span>
              <p className="text-slate-700 dark:text-slate-300 text-sm md:text-[15px] leading-relaxed font-medium italic relative z-10 px-4 pt-4 pb-2">
                {details.bio}
              </p>
              <span className="absolute bottom-2 right-2 text-4xl text-slate-200 dark:text-slate-700 opacity-50 font-serif rotate-180">"</span>
            </div>
            <div className="mt-5 pt-5 border-t border-slate-200 dark:border-slate-800 flex items-center justify-between">
              <div className="flex flex-col">
                <p className="text-[9px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest mb-0.5">Integration Date</p>
                <p className="text-xs md:text-sm font-bold text-slate-900 dark:text-white flex items-center gap-1.5">
          ...
          <motion.div variants={fadeUp} initial="hidden" animate="show" className="md:col-span-3 grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
            {[
              ...
              <motion.div key={i} whileHover={{ y: -5, scale: 1.02 }} className={`${stat.bg} p-5 sm:p-6 rounded-[2rem] border ${stat.border} shadow-sm flex flex-col relative overflow-hidden group`}>
                <div className="absolute -right-4 -top-4 text-5xl opacity-10 group-hover:scale-125 group-hover:rotate-12 transition-transform duration-500 grayscale">{stat.icon}</div>
                ...
                <p className={`text-2xl sm:text-3xl font-black ${stat.color} truncate w-full relative z-10 drop-shadow-sm`}>{stat.val}</p>
              </motion.div>
            ))}
          </motion.div>
        ...
        <motion.div variants={staggerContainer} initial="hidden" animate="show" className="bg-white/70 dark:bg-slate-900/60 backdrop-blur-2xl rounded-[2.5rem] md:rounded-[3rem] p-6 sm:p-10 border border-white/80 dark:border-slate-700/50 shadow-xl dark:shadow-[0_8px_30px_rgb(0,0,0,0.2)]">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
            <h3 className="text-xl md:text-2xl font-black text-slate-900 dark:text-white uppercase tracking-tight flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl bg-indigo-50 dark:bg-indigo-500/20 flex items-center justify-center border border-indigo-100 dark:border-indigo-500/30 text-indigo-500">
            ...
            <span className="bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 text-[10px] md:text-xs font-black uppercase tracking-widest px-4 py-2 rounded-xl shadow-inner border border-slate-200/50 dark:border-slate-700/50">
              {myComments.length} Records Found
            </span>
          </div>

          {myComments.length === 0 ? (
            <div className="bg-slate-50/50 dark:bg-slate-800/30 p-10 sm:p-16 rounded-[2rem] border-2 border-dashed border-slate-200 dark:border-slate-700 flex flex-col items-center justify-center text-center">
              <div className="w-20 h-20 bg-white dark:bg-slate-800 rounded-[1.5rem] shadow-sm flex items-center justify-center text-4xl mb-4 border border-slate-100 dark:border-slate-700">📭</div>
              <h4 className="text-lg font-black text-slate-900 dark:text-white mb-2">No Audits Yet</h4>
              <p className="text-slate-500 dark:text-slate-400 font-medium mb-6 text-sm max-w-sm">Your audit history is empty. Start reviewing projects to build your track record.</p>
              <Link to="/explore" className="px-6 py-3 bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 font-black text-xs uppercase tracking-widest rounded-xl hover:bg-indigo-100 dark:hover:bg-indigo-500/20 transition-colors border border-indigo-100 dark:border-indigo-500/20 flex items-center gap-2">
            ...
            <div className="flex flex-col gap-4 md:gap-5">
              {myComments.slice().reverse().map((comment, idx) => (
                <motion.div key={comment.id} variants={fadeUp} className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-md p-5 sm:p-6 rounded-[1.5rem] md:rounded-[2rem] border border-slate-100 dark:border-slate-700/60 shadow-sm flex flex-col gap-4 group hover:shadow-md hover:border-indigo-100 dark:hover:border-indigo-500/30 transition-all duration-300">
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                    <div className="flex items-center gap-2">
                      <div className="w-8 h-8 rounded-lg bg-slate-50 dark:bg-slate-900 border border-slate-200/60 dark:border-slate-700/60 flex items-center justify-center text-slate-400 dark:text-slate-500">
                        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                      </div>
                      <span className="text-[10px] md:text-[11px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest">Logged: {formatDateTime(comment.created_at)}</span>
                    </div>
                    {Number(comment.is_ghost_alert) === 1 ? (
                      <span className="bg-rose-50 dark:bg-rose-500/10 text-rose-600 dark:text-rose-400 text-[10px] font-black uppercase tracking-widest px-3 py-1.5 rounded-lg border border-rose-100 dark:border-rose-500/20 flex items-center gap-1.5 shadow-sm">
                        <span className="w-2 h-2 rounded-full bg-rose-500 animate-pulse"></span>
                        Ghost Flag
                      </span>
                    ) : (
                      <span className="bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 text-[10px] font-black uppercase tracking-widest px-3 py-1.5 rounded-lg border border-indigo-100 dark:border-indigo-500/20 flex items-center gap-1.5 shadow-sm">
                  ...
                  <Link to={`/project/${comment.project_id}`} className="text-[11px] md:text-xs font-black uppercase tracking-widest text-indigo-500 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors w-fit flex items-center gap-2 mt-1 group-hover:translate-x-2 duration-300 bg-transparent px-0">
                    Inspect Record
                    <div className="w-6 h-6 rounded-full bg-indigo-50 dark:bg-indigo-500/20 flex items-center justify-center group-hover:bg-indigo-100 dark:group-hover:bg-indigo-500/40 transition-colors">
          ...
          {isEditing && (
            <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-0 sm:p-4 bg-slate-900/40 dark:bg-black/60 backdrop-blur-md">
              <motion.div
                ...
                className="bg-white/95 dark:bg-slate-900/95 backdrop-blur-2xl p-6 sm:p-10 rounded-t-[2.5rem] sm:rounded-[2.5rem] border border-white/50 dark:border-slate-700/50 w-full max-w-md shadow-2xl h-[85vh] sm:h-auto flex flex-col relative"
              >
                <button onClick={() => setIsEditing(false)} className="absolute top-6 right-6 w-8 h-8 bg-slate-100 dark:bg-slate-800 rounded-full flex items-center justify-center text-slate-500 hover:text-slate-900 dark:hover:text-white transition-colors hidden sm:flex">
                ...
                <h2 className="text-2xl font-black mb-2 text-slate-900 dark:text-white uppercase tracking-tight text-center">Edit Identity</h2>
                <p className="text-xs text-center text-slate-500 dark:text-slate-400 font-medium mb-8">Update your public profile information</p>

                <div className="flex flex-col gap-6 flex-1 overflow-y-auto scrollbar-hide pb-10 sm:pb-0 px-1">
                  <div>
                    <label className="text-[10px] sm:text-xs font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest ml-2 mb-2 flex items-center gap-1.5">
                      <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
                      Region / City
                    </label>
                    <input type="text" placeholder="e.g. Obando, Bulacan" value={details.location} onChange={(e) => setDetails({ ...details, location: e.target.value })} className="w-full p-4 rounded-2xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-all text-slate-900 dark:text-white shadow-inner font-medium placeholder:text-slate-400" />
                  </div>
                  <div>
                    <label className="text-[10px] sm:text-xs font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest ml-2 mb-2 flex items-center gap-1.5">
                    ...
                    <textarea placeholder="Your advocacy, profession, or passion..." value={details.bio} onChange={(e) => setDetails({ ...details, bio: e.target.value })} className="w-full p-4 rounded-2xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-all resize-none text-slate-900 dark:text-white h-32 shadow-inner font-medium placeholder:text-slate-400 leading-relaxed" rows={4} />
                  </div>
                  <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 mt-auto sm:mt-4 pt-4 border-t border-slate-100 dark:border-slate-800">
                    <button onClick={() => setIsEditing(false)} disabled={isSaving} className="w-full py-4 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 rounded-2xl font-black text-xs uppercase tracking-widest disabled:opacity-50 transition-colors hover:bg-slate-200 dark:hover:bg-slate-700">Cancel</button>
                    <button onClick={handleSaveProfile} disabled={isSaving} className="w-full py-4 bg-indigo-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-lg shadow-indigo-500/30 disabled:opacity-50 transition-transform active:scale-95 flex justify-center items-center gap-2">
                      {isSaving ? (
                        <>
