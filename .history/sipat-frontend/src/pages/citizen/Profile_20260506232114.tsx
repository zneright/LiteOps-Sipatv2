...
export default function Profile() {
  const { user } = useAuth(); // Ensure user object contains 'uid' (firebase_uid)
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        if (user?.uid) {
          const userRes = await fetch(`http://localhost:8080/api/users?firebase_uid=${user.uid}`);
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-white dark:bg-slate-900 p-8 rounded-[2rem] border border-slate-200 dark:border-slate-800 w-full max-w-md shadow-2xl">
              ...
              <div className="flex flex-col gap-4">
                ...
                <div className="flex gap-3 mt-4">
                  <button onClick={() => setIsEditing(false)} disabled={isSaving} className="flex-1 py-3 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 rounded-xl font-bold text-sm disabled:opacity-50">Cancel</button>
                  <button onClick={handleSaveProfile} disabled={isSaving} className="flex-1 py-3 bg-indigo-600 text-white rounded-xl font-bold text-sm shadow-lg shadow-indigo-500/20 disabled:opacity-50">
