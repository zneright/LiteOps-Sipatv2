...
interface Project {
  ...
  file_url: string;
  phases: any;
  // 🚀 NEW: Added reaction types to the interface
  likes?: number;
  neutrals?: number;
  unlikes?: number;
}
...
export default function Explore() {
  ...
  // 🚀 FIX: Wrapped in useCallback so we can call it after reacting
  ...
  // 🚀 NEW: Handle Reactions directly from the feed
  const handleReaction = async (projectId: number, type: 'likes' | 'neutrals' | 'unlikes') => {
    ...
    try {
      ...
      // Refresh the feed to get updated numbers
      fetchProjects();
    } catch (error) {
                ...
                <div className="p-6 flex flex-col flex-grow gap-4">
                  ...
                  <div className="flex flex-col gap-1.5 mt-1">
                    <div className="flex justify-between items-end">
                      ...
                      <span className={`text-[10px] font-bold truncate max-w-[60%] text-right ${percentage === 100 ? 'text-emerald-500' : 'text-indigo-600 dark:text-indigo-400'}`}>
                  ...
                  <p className="text-sm text-slate-500 dark:text-slate-400 line-clamp-2 flex-grow mt-2">
                    {project.description}
                  </p>
                  ...
                  <div className="flex items-center justify-between mt-2 pt-4 border-t border-slate-100 dark:border-slate-800">
                    ...
                    {/* 🚀 NEW: Animated Reaction Buttons integrated into the footer layout */}
                    <div className="flex items-center gap-3">
                      <motion.button
                        whileHover={{ scale: 1.15 }}
                        whileTap={{ scale: 0.8 }}
                        onClick={() => handleReaction(project.id, 'likes')}
                      ...
                      <motion.button
                        whileHover={{ scale: 1.15 }}
                        whileTap={{ scale: 0.8 }}
                        onClick={() => handleReaction(project.id, 'neutrals')}
                      ...
                      <motion.button
                        whileHover={{ scale: 1.15 }}
                        whileTap={{ scale: 0.8 }}
                        onClick={() => handleReaction(project.id, 'unlikes')}
                  ...
                  {/* View Details Button moved slightly down for better spacing */}
                  <Link
                    to={`/project/${project.id}`}
                    className="mt-2 text-center w-full bg-slate-50 hover:bg-slate-100 dark:bg-slate-800 dark:hover:bg-slate-700 text-indigo-600 dark:text-indigo-400 text-xs font-bold py-2.5 rounded-xl transition-colors"
                  >
                    View Full Details
                  </Link>
