...
export default function ProjectView() {
    ...
    const ui = getExactCategoryUI(project.category);
        ...
        return (
            <div key={comment.id} className="relative z-10">
                ...
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className={` ${isReply ? 'ml-6 sm:ml-8 mt-4' : 'mt-8'} p-5 sm:p-6 rounded-[1.5rem] sm:rounded-[2rem] border transition-all duration-300 shadow-[0_4px_20px_rgb(0,0,0,0.03)] dark:shadow-[0_4px_20px_rgb(0,0,0,0.2)] relative z-10 ${isGhost ? 'bg-rose-50/80 dark:bg-rose-900/10 border-rose-200/60 dark:border-rose-500/20' : comment.ai_match_score >= 40 ? 'bg-emerald-50/80 dark:bg-emerald-900/10 border-emerald-200/60 dark:border-emerald-500/20' : 'bg-white/80 dark:bg-slate-800/60 border-slate-100 dark:border-slate-700/50 backdrop-blur-xl'} `}>
                    ...
                    <div className="flex flex-wrap items-center justify-between border-t border-slate-200/60 dark:border-slate-700/50 pt-4 mt-2">
                        <div className="flex items-center gap-1.5 bg-slate-50 dark:bg-slate-800/50 p-1.5 rounded-[1rem] border border-slate-100 dark:border-slate-700/50 shadow-sm">
                        ...
                        <button onClick={() => initiateReply(comment.id, comment.author_name)} className="text-[11px] font-black uppercase tracking-widest text-indigo-500 hover:text-indigo-600 dark:hover:text-indigo-400 flex items-center gap-2 px-4 py-2 bg-indigo-50 dark:bg-indigo-500/10 hover:bg-indigo-100 dark:hover:bg-indigo-500/20 rounded-xl transition-all duration-300 transform active:scale-95 group/reply border border-indigo-100 dark:border-indigo-500/20">
            ...
            <div className="relative max-w-[90rem] mx-auto py-8 sm:py-12 px-4 sm:px-6 lg:px-8 flex flex-col xl:flex-row gap-8 xl:gap-12">
                ...
                <div className="w-full xl:w-[55%] flex flex-col gap-8">
                    <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ type: "spring", bounce: 0.4 }} className="flex flex-col gap-8">
                        ...
                        <div className="bg-white/60 dark:bg-slate-900/60 backdrop-blur-2xl rounded-[2rem] md:rounded-[2.5rem] p-3 sm:p-4 flex flex-row overflow-x-auto snap-x scrollbar-hide gap-3 border border-white/80 dark:border-slate-700/50 shadow-xl dark:shadow-[0_8px_30px_rgb(0,0,0,0.2)] relative z-20">
                            <div className="absolute left-0 top-0 bottom-0 w-8 bg-gradient-to-r from-white dark:from-slate-900 to-transparent z-10 pointer-events-none md:hidden rounded-l-[2rem]"></div>
                            <div className="absolute right-0 top-0 bottom-0 w-8 bg-gradient-to-l from-white dark:from-slate-900 to-transparent z-10 pointer-events-none md:hidden rounded-r-[2rem]"></div>

                            <button onClick={() => setActiveTab("Overall Project")} className={`shrink-0 md:flex-1 text-center px-6 py-3.5 sm:py-4 rounded-[1.25rem] text-[11px] sm:text-xs font-black uppercase tracking-widest transition-all duration-300 snap-center relative ${activeTab === "Overall Project" ? "text-white shadow-[0_4px_15px_rgba(99,102,241,0.4)] transform scale-[1.02]" : "text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-white/50 dark:hover:bg-slate-800/50"}`}>
                                {activeTab === "Overall Project" && (
                                    <motion.div layoutId="projectTab" className="absolute inset-0 bg-gradient-to-r from-indigo-500 to-purple-600 rounded-[1.25rem] -z-10" transition={{ type: "spring", bounce: 0.2, duration: 0.6 }} />
                                )}
                                <span className="relative z-10">Overall Scope</span>
                            </button>
                            {project.phases?.map((p: any) => (
                                <button key={p.title} onClick={() => setActiveTab(p.title)} className={`shrink-0 md:flex-1 flex flex-col justify-center items-center px-6 py-2.5 sm:py-3 rounded-[1.25rem] transition-all duration-300 snap-center relative ${activeTab === p.title ? "text-white shadow-[0_4px_15px_rgba(99,102,241,0.4)] transform scale-[1.02]" : "text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-white/50 dark:hover:bg-slate-800/50"}`}>
                                    {activeTab === p.title && (
                                        <motion.div layoutId="projectTab" className="absolute inset-0 bg-gradient-to-r from-indigo-500 to-purple-600 rounded-[1.25rem] -z-10" transition={{ type: "spring", bounce: 0.2, duration: 0.6 }} />
                                    )}
                                    <div className="flex items-center gap-2 relative z-10">
                                        <span className="text-[11px] sm:text-xs font-black uppercase tracking-widest truncate max-w-[120px]">{p.title}</span>
                                        {(p.status === "completed" || (p.image_url && p.image_url.trim() !== "")) && <svg className={`w-3.5 h-3.5 shrink-0 ${activeTab === p.title ? "text-white" : "text-emerald-500"}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" /></svg>}
                                    </div>
                                </button>
                            ))}
                        </div>
