...
// 🚀 EXACT MATCH METADATA (Synced with Categories.tsx)
...
export default function Explore() {
  ...
  const getExactCategoryUI = (categoryName: string) => {
    ...
    return { bg: "from-slate-500 to-slate-700", icon: "📁" }; // Fallback for uncategorized
  };
          ...
          {projects.map((project, idx) => {
            ...
            return (
              <motion.div key={project.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.05 }} className="group bg-white rounded-[2rem] border border-slate-200/60 overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col relative hover:-translate-y-1">
                ...
                <div className="p-6 flex flex-col flex-grow gap-3 relative z-10 bg-white">
                  <div>
                  ...
                  {/* 🚀 Agency Link */}
                  <div className="mt-2">
                    <Link to={`/agency-profile/${encodeURIComponent(orgName)}`} className="flex items-center gap-3 w-full bg-slate-50 hover:bg-indigo-50/50 p-2.5 rounded-xl border border-slate-100 hover:border-indigo-100 transition-all group/agency">
                      <div className="w-8 h-8 rounded-lg bg-white text-indigo-600 border border-slate-200 shadow-sm flex items-center justify-center text-xs font-black group-hover/agency:scale-110 group-hover/agency:bg-indigo-600 group-hover/agency:text-white transition-all">
                        {orgName.charAt(0).toUpperCase()}
                      </div>
                      <div className="flex flex-col">
                        <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">Managed By</span>
                        <span className="text-xs font-bold text-slate-700 group-hover/agency:text-indigo-700 transition-colors leading-none">{orgName}</span>
                      </div>
                    </Link>
                  </div>

                  <div className="flex items-center justify-between mt-3 pt-4 border-t border-slate-100">
                    <div className="flex flex-col">
