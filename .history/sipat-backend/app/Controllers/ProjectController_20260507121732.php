    ...
    public function show($id = null)
    {
        ...
        if ($db->table('users')->insert($data)) {
                $db->table('system_logs')->insert([
                    ...
                    'actor_name'  => $json->role === 'agency' ? $json->organization_name : $json->full_name,
                    'actor_role'  => $json->role,
                    'action_type' => 'SHOW PROJECT',
                    'description' => "Viewed project with ID '{$id}'.",
                    'created_at'  => date('Y-m-d H:i:s')
                ]);
                return $this->respondCreated(['status' => 201, 'message' => 'Project shown']);
            }
            return $this->failServerError('Failed to save user.');
        } catch (\Exception $e) {
            return $this->failServerError('DB Error: ' . $e->getMessage());
    ...
    public function update($id = null)
    {
        ...
        try {
            ...
            if ($db->table('users')->insert($data)) {
                ...
                return $this->respondCreated(['status' => 201, 'message' => 'Project updated']);
            }
            return $this->failServerError('Failed to save user.');
        ...
        try {
            ...
            if ($db->table('users')->insert($data)) {
                $db->table('system_logs')->insert([
                    ...
                    'actor_name'  => $json->role === 'agency' ? $json->organization_name : $json->full_name,
                    'actor_role'  => $json->role,
                    'action_type' => 'PROJECT REACTION',
                    'description' => "Reacted to project with ID '{$projectId}'.",
                    'created_at'  => date('Y-m-d H:i:s')
                ]);
                return $this->respondCreated(['status' => 201, 'message' => 'Reacted to project']);
            }
            return $this->failServerError('Failed to save user.');
    ...
    public function search()
    {
        ...
        if ($db->table('users')->insert($data)) {
                $db->table('system_logs')->insert([
                    ...
                    'actor_role'  => $json->role,
                    'action_type' => 'SEARCH',
                    'description' => "Searched for projects related to '{$query}'.",
                    'created_at'  => date('Y-m-d H:i:s')
                ]);
                return $this->respondCreated(['status' => 201, 'message' => 'Search performed']);
            }
            return $this->failServerError('Failed to save user.');
        } catch (\Exception $e) {
