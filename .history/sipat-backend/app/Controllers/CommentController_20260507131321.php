...
class ProjectController extends BaseController
{
    ...
    public function create()
    {
        ...
        try {
            ...
            $data = [
                ...
                'organization_name'=> $json->organization_name ?? 'Agency', 
            ...
            if ($db->table('projects')->insert($data)) {
                ...
                $db->table('system_logs')->insert([
                    'actor_email' => 'system@sipat.com',
                    'actor_name'  => $data['organization_name'],
                    'actor_role'  => 'agency',
                    'action_type' => 'PROJECT CREATION',
                    'description' => "Created a new project: {$data['title']}",
                    'created_at'  => date('Y-m-d H:i:s')
                ]);
    ...
    public function update($id = null)
    {
        ...
        try {
            $db = \Config\Database::connect();
            if ($db->table('projects')->where('id', $id)->update(['phases' => $json->phases])) {
                $db->table('system_logs')->insert([
                    'actor_email' => 'system@sipat.com',
                    'actor_name'  => 'Agency',
                    'actor_role'  => 'agency',
                    'action_type' => 'PROJECT UPDATE',
                    'description' => "Updated project timeline phases for ID '{$id}'.",
                    'created_at'  => date('Y-m-d H:i:s')
                ]);
    ...
    public function react($projectId = null)
    {
        ...
        try {
            $db = \Config\Database::connect();
            $existing = $db->table('project_reactions')->where('project_id', $projectId)->where('user_email', $email)->get()->getRowArray();

            if ($existing) {
                if ($existing['reaction_type'] === $json->type) {
                    return $this->respond(['status' => 200, 'message' => 'Already reacted']);
                } else {
                    ...
                    $db->table('system_logs')->insert(['actor_email' => $email, 'actor_name' => 'Citizen', 'actor_role' => 'citizen', 'action_type' => 'PROJECT REACTION', 'description' => "Switched reaction to '{$json->type}' on project #{$projectId}.", 'created_at' => date('Y-m-d H:i:s')]);
                    return $this->respond(['status' => 200, 'message' => 'Reaction switched']);
                }
            }

            $db->query("UPDATE projects SET {$json->type} = {$json->type} + 1 WHERE id = ?", [$projectId]);
            $db->table('project_reactions')->insert(['project_id' => $projectId, 'user_email' => $email, 'reaction_type' => $json->type]);
            
            $db->table('system_logs')->insert(['actor_email' => $email, 'actor_name' => 'Citizen', 'actor_role' => 'citizen', 'action_type' => 'PROJECT REACTION', 'description' => "Reacted '{$json->type}' to project #{$projectId}.", 'created_at' => date('Y-m-d H:i:s')]);
    ...
    public function search()
    {
        ...
        if (!$query) return $this->respond(['projects' => [], 'users' => []]);
        ...
        try {
            ...
            if (!empty($commentProjectIds)) {
                $commentProjects = $db->table('projects')->whereIn('id', $commentProjectIds)->get()->getResultArray();
