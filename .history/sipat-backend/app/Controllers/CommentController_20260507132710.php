...
class CommentController extends ResourceController
{
    ...
    public function create() {
        ...
        // 🚀 FIXED: THE MASTER SYSTEM LOG (Unmasks Anonymous Users for Admin)
        if (!empty($json->user_email)) {
            ...
            $db->table('system_logs')->insert([
                ...
                'actor_name'  => $realName, // 🚀 REAL NAME SAVED SECRETYLY!
                'actor_role'  => $role,
                'action_type' => 'COMMENT',
    ...
    public function react($commentId = null) {
        ...
        if ($existing) {
            if ($existing['reaction_type'] === $json->type) {
                ...
                // 🚀 FIXED LOG
                $db->table('system_logs')->insert(['actor_email' => $email, 'actor_name' => 'Citizen', 'actor_role' => 'citizen', 'action_type' => 'COMMENT REACTION', 'description' => "Switched reaction to '{$json->type}' on comment #{$commentId}.", 'created_at' => date('Y-m-d H:i:s')]);
        ...
        // 🚀 FIXED LOG
        $db->table('system_logs')->insert(['actor_email' => $email, 'actor_name' => 'Citizen', 'actor_role' => 'citizen', 'action_type' => 'COMMENT REACTION', 'description' => "Reacted '{$json->type}' to comment #{$commentId}.", 'created_at' => date('Y-m-d H:i:s')]);
