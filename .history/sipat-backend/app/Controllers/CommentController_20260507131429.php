...
class CommentController extends ResourceController
{
    ...
    public function create() {
        ...
        // 2. 🚀 THE MASTER SYSTEM LOG (Unmasks Anonymous Users)
        if (!empty($json->user_email)) {
            ...
            $db->table('system_logs')->insert([
                ...
                'actor_name'  => $realName, // REAL NAME SAVED!
                'actor_role'  => $role,
                'action_type' => 'COMMENT',
    ...
    public function react($commentId = null) {
        ...
        if ($existing) {
            if ($existing['reaction_type'] === $json->type) {
                ...
                // Log Reaction Switch
                $db->table('system_logs')->insert(['actor_email' => $email, 'actor_name' => 'User', 'actor_role' => 'citizen', 'action_type' => 'COMMENT REACTION', 'description' => "Switched reaction to '{$json->type}' on comment #{$commentId}.", 'created_at' => date('Y-m-d H:i:s')]);
        ...
        // Log New Reaction
        $db->table('system_logs')->insert(['actor_email' => $email, 'actor_name' => 'User', 'actor_role' => 'citizen', 'action_type' => 'COMMENT REACTION', 'description' => "Reacted '{$json->type}' on comment #{$commentId}.", 'created_at' => date('Y-m-d H:i:s')]);
