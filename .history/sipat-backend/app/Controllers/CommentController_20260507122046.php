    ...
    public function create() {
        ...
        if ($db->table('users')->insert($data)) {
                $db->table('system_logs')->insert([
                    ...
                    'action_type' => 'COMMENT',
                    'description' => "Posted a new comment on Project #" . $json->project_id,
                    'created_at'  => date('Y-m-d H:i:s')
                ]);
                return $this->respondCreated(['status' => 201, 'message' => 'Comment posted & Logged']);
            }
        ...
        if ($existing) {
            ...
            if ($db->table('users')->insert($data)) {
                $db->table('system_logs')->insert([
                    ...
                    'actor_name'  => $json->role === 'agency' ? $json->organization_name : $json->full_name,
                    'actor_role'  => $json->role,
                    'action_type' => 'COMMENT REACTION',
                    'description' => "REACTED to comment with ID '{$commentId}' using reaction type '{$json->type}'.",
                    'created_at'  => date('Y-m-d H:i:s')
                ]);
                return $this->respondCreated(['status' => 201, 'message' => 'Reacted to comment']);
            }
            return $this->failServerError('Failed to save user.');
        }
