...
class UserController extends BaseController
{
    ...
    public function toggleSaveProject() {
        ...
        if (in_array($projectId, $saved)) {
            $saved = array_values(array_diff($saved, [$projectId])); 
            $status = 'removed';
        } else {
            $saved[] = $projectId;
            $status = 'added';
    ...
    public function toggleFollowAgency() {
        ...
        if (in_array($agencyName, $followed)) {
            $followed = array_values(array_diff($followed, [$agencyName])); 
            $status = 'unfollowed';
        } else {
            $followed[] = $agencyName; 
            $status = 'followed';
        ...
        try {
            ...
            if ($db->table('users')->insert($data)) {
                $db->table('system_logs')->insert([
                    ...
                    'description' => "Toggled active status for user with Firebase UID '{$firebaseUid}'.",
                    'created_at'  => date('Y-m-d H:i:s')
                ]);
                return $this->respondCreated(['status' => 201, 'message' => 'User status updated']);
            }
            return $this->failServerError('Failed to save user.');
