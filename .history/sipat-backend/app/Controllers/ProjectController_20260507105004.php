...
class ProjectController extends BaseController
{
   ...
   public function create()
    ...
    public function search()
    {
        ...
        if (!$query) {
            return $this->respond([]); // Return empty if no query
        }

        $db = \Config\Database::connect();
        
        // 1. Search Projects (Matches title, description, keywords, OR ai_summary)
        $projects = $db->table('projects')
            ->groupStart()
                ->like('title', $query, 'both', null, true)
                ->orLike('description', $query, 'both', null, true)
                ->orLike('keywords', $query, 'both', null, true)
                ->orLike('ai_summary', $query, 'both', null, true) // 🚀 Searches the scanned PDF content!
                ->orLike('organization_name', $query, 'both', null, true) // Match by agency name too
            ->groupEnd()
            ->orderBy('created_at', 'DESC')
            ->get()->getResultArray();

        // 2. Search Agencies (Matches organization name or bio)
