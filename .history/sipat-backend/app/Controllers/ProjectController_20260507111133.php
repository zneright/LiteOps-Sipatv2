...
class ProjectController extends BaseController
{
   ...
   public function create()
    ...
    public function search()
    {
        ...
        // 1. Search Projects (Matches title, description, keywords, OR ai_summary from PDFs)
        $projects = $db->table('projects')
            ->groupStart()
                ->like('title', $query, 'both', null, true)
                ->orLike('description', $query, 'both', null, true)
                ->orLike('keywords', $query, 'both', null, true)
                ->orLike('ai_summary', $query, 'both', null, true)
                ->orLike('organization_name', $query, 'both', null, true)
            ->groupEnd()
            ->orderBy('created_at', 'DESC')
            ->get()->getResultArray();

        // 2. Search Comments (Find projects based on citizen feedback/Cloudinary uploads!)
        ...
        if (!empty($commentProjectIds)) {
            ...
            foreach ($commentProjects as $cp) {
                if (!in_array($cp['id'], $existingIds)) {
                    // Tag it so the frontend knows it was found via comment
